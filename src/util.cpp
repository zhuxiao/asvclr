...
// set the current maximum open files
bool setMaxOpenFileNum(rlim_t newLimit) {
	struct rlimit limit;
	limit.rlim_cur = newLimit;  // set the soft limits
	limit.rlim_max = newLimit;  // set the hard limits (if necessary)
...
reg_t* dupVarReg(reg_t *reg){
	...
	if(reg){
		...
		reg_ret->merge_flag = reg->merge_flag;
		reg_ret->rescue_flag = reg->rescue_flag;
...
bool isOverlappedPos(size_t startPos1, size_t endPos1, size_t startPos2, size_t endPos2){
	bool flag = false;
	size_t tmp;
	if(startPos1>endPos1) { tmp = startPos1; startPos1 = endPos1; endPos1 = tmp; }
	if(startPos2>endPos2) { tmp = startPos2; startPos2 = endPos2; endPos2 = tmp; }
...
int32_t getOverlapSize(size_t startPos1, size_t endPos1, size_t startPos2, size_t endPos2){
	...
	if(isOverlappedPos(startPos1, endPos1, startPos2, endPos2))
		overlap_size = min(endPos1, endPos2) - max(startPos1, startPos2);
	else overlap_size = 0;
...
bool isAdjacent(size_t startPos1, size_t endPos1, size_t startPos2, size_t endPos2, int32_t dist_thres){
	...
	if(overlap_size!=0 and overlap_size>=-dist_thres) flag = true;
	else flag = false;
...
bool isIndelInSingleClipReg(reg_t *reg, mateClipReg_t *clip_reg){
	...
	if(clip_reg->valid_flag){
		if(clip_reg->reg_mated_flag){ // mated
			if(clip_reg->sv_type==VAR_DUP or clip_reg->sv_type==VAR_INV){ // DUP or INV
				...
				if(reg->chrname.compare(chrname_tmp)==0 and chrname_tmp.compare(chrname_tmp2)==0 and start_pos>0 and end_pos>0)
					if(isOverlappedPos(reg->startRefPos, reg->endRefPos, start_pos, end_pos)) flag = true;
		...
		}else if(clip_reg->large_indel_flag){ // large indel
			//reg_tmp = clip_reg->leftClipReg ? clip_reg->leftClipReg : clip_reg->rightClipReg; // deleted on 2025-09-17
			reg_tmp = clip_reg->largeIndelClipReg;
			chrname_tmp = reg_tmp->chrname;
			start_pos = reg_tmp->startRefPos;
			end_pos = reg_tmp->endRefPos;
			if(reg->chrname.compare(chrname_tmp)==0 and start_pos>0 and end_pos>0)
...
// determine whether the region in a clipping region
	...
	if(clip_reg->valid_flag){
		if(clip_reg->reg_mated_flag){ // mated
			if(clip_reg->sv_type==VAR_DUP or clip_reg->sv_type==VAR_INV){ // DUP or INV
				...
				if(chrname.compare(chrname_tmp)==0 and chrname_tmp.compare(chrname_tmp2)==0 and start_pos>0 and end_pos>0 and abs(dist)<maxVarRegSize)
					if(isOverlappedPos(start_pos_para, end_pos_para, start_pos, end_pos)) flag = true;
			}else if(clip_reg->sv_type==VAR_TRA){  // TRA
				// check left part
				chrname_tmp = "";
				start_pos = end_pos = 0;
				...
				if(chrname.compare(chrname_tmp)==0 and start_pos>0 and end_pos>0 and abs(dist)<maxVarRegSize)
					if(isOverlappedPos(start_pos_para, end_pos_para, start_pos, end_pos)) flag = true;
...
vector<int32_t> getStartEndIndexInClipRegVec(string &chrname, size_t start_pos, size_t end_pos, vector<mateClipReg_t*> &mate_clipReg_vec, int64_t maxVarRegSize){
	...
	int32_t i, start_idx, end_idx;
	bool flag;
	mateClipReg_t *clip_reg;

	start_idx = end_idx = -1;
	if(mate_clipReg_vec.size()>0){
		for(i=0; i<(int32_t)mate_clipReg_vec.size(); i++){ // get the left overlapped region
			clip_reg = mate_clipReg_vec.at(i);
			flag = isRegInSingleClipReg(chrname, start_pos, end_pos, clip_reg, maxVarRegSize);
			if(flag) { start_idx = i; break; }
		}
		for(i=(int32_t)mate_clipReg_vec.size()-1; i>=0; i--){ // get the right overlapped region
			clip_reg = mate_clipReg_vec.at(i);
			flag = isRegInSingleClipReg(chrname, start_pos, end_pos, clip_reg, maxVarRegSize);
			if(flag) { end_idx = i; break; }
		}
	}

	ret_vec.push_back(start_idx);
	ret_vec.push_back(end_idx);
...
void printMateClipReg(mateClipReg_t *mate_clip_reg){
	string chrname_left, chrname_right, bnd_mate_reg_str;
	...
	bnd_mate_reg_str = mate_clip_reg->bnd_mate_reg_strs[0];
	for(int32_t i=1; i<4; i++) bnd_mate_reg_str += ";" + mate_clip_reg->bnd_mate_reg_strs[i];

	cout << chrname_left << ":" << start_pos_left << "-" << end_pos_left << ", " << chrname_right << ":" << start_pos_right << "-" << end_pos_right << " :" << endl;
	cout << "\tvalid_flag=" << mate_clip_reg->valid_flag << ", call_success_flag=" << mate_clip_reg->call_success_flag << ", large_indel_flag=" << mate_clip_reg->large_indel_flag << ", ularge_flag=" << mate_clip_reg->ultra_large_dist_flag << ", dist_breakpoint=" << to_string(mate_clip_reg->dist_breakpoint) << ", leftClipRegNum=" << to_string(mate_clip_reg->leftClipRegNum) << ", rightClipRegNum=" << to_string(mate_clip_reg->rightClipRegNum) << ", sv_type=" << mate_clip_reg->sv_type << ", dup_num=" << mate_clip_reg->dup_num << ", bnd_mate_reg_str= " << bnd_mate_reg_str << endl;
}
...
vector<struct querySeqInfoNode*> extractQueriesFromClipAlnDataVec(vector<clipAlnData_t*> &clipAlnDataVector, const string &refseq_given, string &chrname, int64_t startRefPos, int64_t endRefPos, int64_t startRefPos_core, int64_t endRefPos_core, faidx_t *fai, int32_t minConReadLen, bool clip_reg_flag, pthread_mutex_t *p_mutex_fai){
	...
	if(clipAlnDataVector.size()>0){
		...
		if(bam_type!=BAM_INVALID){
			// extract query clip align segments
					...
					if(clip_reg_flag==false){
						...
						if(noHardClipIdx!=-1){
							...
							if(query_seq_qual_vec.size()>0 and query_seq_qual_vec.at(0).length()>(size_t)minConReadLen and query_aln_segs.at(noHardClipIdx)->bam){
								...
								q_node->cluster_finished_flag = q_node->qcSig_merge_flag = false;
								...
								removeHeadTailAlnSegs(alnSegs, false);
...
int32_t getNoHardClipAlnItem(vector<clipAlnData_t*> &query_aln_segs, string &chrname, int64_t startRefPos_core, int64_t endRefPos_core){
	...
	// prefer the entire flanking
	idx = -1;
	for(i=0; i<query_aln_segs.size(); i++){
		clip_aln = query_aln_segs.at(i);
		if(clip_aln->chrname.compare(chrname)==0 and clip_aln->startRefPos<startRefPos_core and clip_aln->endRefPos>endRefPos_core and clip_aln->leftHardClippedFlag==false and clip_aln->rightHardClippedFlag==false){
			idx = i;
			break;
		}
	}
...
//		cout << "qname=" << qname << endl;
		...
		if(query_alnSegs.size()>0){
			switch (op) {
				...
				case BAM_CSOFT_CLIP:
				case BAM_CHARD_CLIP:
					//target_startQpos = query_alnSegs.at(0)->startQpos + query_alnSegs.at(0)->seglen; // deleted on 2024-03-27
					target_startQpos = query_alnSegs.at(0)->startQpos;
					delete *query_alnSegs.begin();
					query_alnSegs.erase(query_alnSegs.begin());
					break;
...
void performLocalCons(string &readsfilename, string &contigfilename, string &refseqfilename, string &clusterfilename, string &tmpdir, string &technology, string &pg_runid_str, double min_seqsim_match, int32_t sv_len_est, size_t num_threads_per_cns_work, vector<reg_t*> &varVec, string &chrname, string &inBamFile, faidx_t *fai, int32_t cns_extend_size, ofstream &cns_info_file, double expected_cov_cns, double min_input_cov_canu, double max_ultra_high_cov, int32_t minMapQ, int32_t minHighMapQ, bool delete_reads_flag, bool keep_failed_reads_flag, bool clip_reg_flag, int32_t minClipEndSize, int32_t maxVarRegSize, int32_t minConReadLen, int32_t min_sv_size, int32_t min_supp_num, double max_seg_size_ratio, double max_seg_nm_ratio, double max_absig_density, bool limit_reg_process_flag, vector<simpleReg_t*> &limit_reg_vec){
	...
	localCns local_cns(readsfilename, contigfilename, refseqfilename, clusterfilename, tmpdir, technology, pg_runid_str, min_seqsim_match, sv_len_est, num_threads_per_cns_work, varVec, chrname, inBamFile, fai, cns_extend_size, expected_cov_cns, min_input_cov_canu, max_ultra_high_cov, minMapQ, minHighMapQ, delete_reads_flag, keep_failed_reads_flag, clip_reg_flag, minClipEndSize, maxVarRegSize, minConReadLen, min_sv_size, min_supp_num, max_seg_size_ratio, max_seg_nm_ratio, max_absig_density);
...
string getUpdatedItemFilename(string &filename, string &out_dir, string &old_out_dir, string &sub_work_dir){
	string new_filename, old_out_dir_tmp;
	...
	if(old_out_dir.size()==0)
		...
		old_out_dir_tmp = getOldOutDirname(filename, sub_work_dir);
		if(old_out_dir_tmp.size()==0)
			new_filename = out_dir + "/" + filename;
		else if(filename.at(old_out_dir_tmp.size())=='/')
			new_filename = out_dir + filename.substr(old_out_dir_tmp.size());
		else
			new_filename = out_dir + "/" + filename.substr(old_out_dir_tmp.size());
	}
...
//		clip_num = base->getTotalClipNum(); // deleted on 2025-03-23
		indel_num = base->getLargeIndelNum(min_indel_size);
		clip_num = base->clipVector.size();
...
bool Filteredbychrname(vector<clipAlnData_t*> &query_aln_segs){
	bool flag = true;
...
//	}
	...
	for(i=0; i<query_aln_segs.size(); ){
		clip_aln = query_aln_segs.at(i);
		//if(query_aln_segs.at(i)->chrname.compare(chrname)!=0)
		if(clip_aln->chrname.compare(chrname)!=0 and clip_aln->MapQ<minMapQ and (clip_aln->ref_dist<min_aln_size or clip_aln->query_dist<min_aln_size)){
...
bool ischeckSameOrient(vector<clipAlnData_t*> &query_aln_segs){
	bool flag = true;
...
bool isQuerySelfOverlap(vector<clipAlnData_t*> &query_aln_segs, int32_t maxVarRegSize){
	...
	for(size_t i=0; i<query_aln_segs.size()-1; i++){
		...
		for(size_t j=i+1; j<query_aln_segs.size(); j++){
			...
			if(flag) break;
			else if(clip_aln1->aln_orient==clip_aln2->aln_orient){ // un-overlapped DUP
				if(clip_aln1->aln_orient==ALN_PLUS_ORIENT and clip_aln1->startQueryPos<clip_aln2->startQueryPos and clip_aln1->endRefPos>clip_aln2->startRefPos){ // plus orient
					flag = true;
					break;
				}else if(clip_aln1->aln_orient==ALN_MINUS_ORIENT and clip_aln1->startQueryPos>clip_aln2->startQueryPos and clip_aln1->endRefPos>clip_aln2->startRefPos){ // minus orient
					flag = true;
					break;
				}
			}
	...
	if(arr_idx!=-1){ // valid based segment
		...
		for(size_t i=0; i<query_aln_segs.size(); i++){
			if(i!=(size_t)arr_idx){
				...
				// filter out ultra-short segments (e.g., <200 bp), added on 2026-01-26
				if(clip_aln->MapQ<min_MapQ_thres and (clip_aln->query_dist<MIN_ALN_SEG_SIZE_CLIPREG or clip_aln->ref_dist<MIN_ALN_SEG_SIZE_CLIPREG) and (double)clip_aln->query_dist/clip_aln->querylen<LONGER_NON_SA_SEG_RATIO_THRES)
					continue;
				...
				if(mate_clip_end==LEFT_END and clip_aln->left_clip_checked_flag==false and clip_aln->leftClipSize>=minClipEndSize){
					...
					//if(abs(ref_dist)<=max_reg_size and abs(dist)<abs(min_dist_same_chr) and clip_aln->chrname.compare(pre_chrname)==0 and clip_aln->aln_orient==clip_aln_based->aln_orient){ // deleted on 2026-01-28
					if(abs(ref_dist)<=max_reg_size and abs(dist)<abs(min_dist_same_chr) and clip_aln->chrname.compare(pre_chrname)==0){
...
vector<int32_t> getAdjacentClipAlnSeg(clipAlnData_t *clip_aln, int32_t clip_end_flag, vector<clipAlnData_t*> &query_aln_segs, int32_t minClipEndSize, int32_t max_reg_size, int32_t min_MapQ_thres){
	...
	adjClipAlnSegInfo = getAdjacentClipAlnSeg(idx_vec, clip_end_flag, query_aln_segs, minClipEndSize, max_reg_size, min_MapQ_thres);
...
// generate BND items
vector<BND_t*> generateBNDItems(int32_t reg_id, int32_t clip_end, int32_t checked_arr[][2], string &chrname1_para, string &chrname2_para, int64_t tra_pos_arr[4], vector<string> &bnd_str_vec, faidx_t *fai, double gt_homo_ratio_thres, double gt_hete_ratio_thres){
	...
	string mate_reg_str2, chrname1, chrname2;
	...
	string id, ref, alt, qual, filter, info, format, format_val_str, sv_type, line_vcf, gt_str;
	double AF;

	if(reg_id<0 or reg_id>=4){
		cerr << __func__ << ", line=" << __LINE__ << ": invalid region id: " << reg_id << ", which should be the integer of [0,3], error." << endl;
		exit(1);
	}

	if(reg_id<=1){
		chrname1 = chrname1_para;
		chrname2 = chrname2_para;
	}else{
		chrname1 = chrname2_para;
		chrname2 = chrname1_para;
	}
	...
	if(tra_pos_arr[reg_id]!=-1 or bnd_str_vec.at(reg_id).compare("-")!=0) {
		...
		if(checked_arr[reg_id][sub_vec_id]==0){ // unprocessed, then process it
			...
			if(mate_reg_str.compare("-")!=0){
				...
				if((reg_id<=1 and mate_reg_id<=1) or (reg_id>=2 and mate_reg_id>=2)){ // invalid same side, then skip
					return bnd_vec;
				}
			}
...
//			}
			...
			if(mate_reg_str.compare("-")!=0){
				...
				bnd_id_str = "ASVCLR.BND." + chrname1 + ":" + to_string(bnd_pos) + "-" + chrname2 + ":" + to_string(mate_bnd_pos);
				mate_bnd_id_str = "ASVCLR.BND." + chrname2 + ":" + to_string(mate_bnd_pos) + "-" + chrname1 + ":" + to_string(bnd_pos);
				mate_dist = -1;
				...
				gt_str = "./.";
				AF = (double)support_num / cov_num;
				if(AF>=gt_homo_ratio_thres) gt_str = GT_HOMOZYGOUS_STR;
				else if(AF>=gt_hete_ratio_thres) gt_str = GT_HETEROZYGOUS_STR;
				else gt_str = GT_NOZYGOUS_STR;
				format_val_str = gt_str + ":" + to_string(cov_num-support_num) + "," + support_num_str + ":" + cov_num_str;
				...
				info += ";DP=" + cov_num_str + ";AF=" + ss.str() + ";LDISCOV=READS";
				...
				format = "GT:AD:DP";		// FORMAT and the values
				gt_str = "./.";
				AF = (double)support_num2 / cov_num2;
				if(AF>=gt_homo_ratio_thres) gt_str = GT_HOMOZYGOUS_STR;
				else if(AF>=gt_hete_ratio_thres) gt_str = GT_HETEROZYGOUS_STR;
				else gt_str = GT_NOZYGOUS_STR;
				format_val_str = gt_str + ":" + to_string(cov_num2-support_num2) + "," + to_string(support_num2) + ":" + to_string(cov_num2);

				info = "SVTYPE=" + sv_type + ";" + "MATEID=" + bnd_id_str;	// INFO: SVTYPE=sv_type;MATEID=mate_bnd_id
				if(chrname1.compare(chrname2)==0) info += ";MATEDIST=" + to_string(mate_dist);  // INFO: MATEDIST=mate_dist
				ss.str(string());
				ss.clear();
				ss << setprecision(3) << (double)AF;
				info += ";DP=" + to_string(cov_num2) + ";AF=" + ss.str() + ";LDISCOV=READS";
...
int64_t getClipPosFromBNDStr(string &bnd_str){
	...
	clip_pos = -1;
	str_vec = split(bnd_str, ",");
	for(i=0; i<str_vec.size(); i++){
		if(str_vec.at(i).compare("-")!=0){
			str_vec2 = split(str_vec.at(i), "|");
			clip_pos = stoi(str_vec2.at(1));
			break;
		}
	}
...
void checkBNDStrVec(mateClipReg_t &mate_clip_reg){
	...
	flag = true;
	//if(mate_clip_reg.valid_flag and mate_clip_reg.sv_type==VAR_TRA){ // TRA, deleted on 2026-01-22
	if(mate_clip_reg.valid_flag and mate_clip_reg.large_indel_flag==false){ // TRA
...
bool isValidBNDStr(int32_t reg_id, int32_t clip_end, int32_t checked_arr[][2], string &chrname1, string &chrname2, int64_t tra_pos_arr[4], vector<string> &bnd_str_vec){
	vector<string> bnd_str_vec2, bnd_str_vec2_tmp, mate_str_vec, mate_str_vec2, mate_str_vec2_tmp;
...
//				cov_num = stoi(cov_num_str);
				...
				if(checked_arr[mate_reg_id][mate_sub_vec_id]==0)
					...
					//return false; // deleted on 2026-01-25
				}
				...
				mate_reg_str2 = bnd_str_vec2_tmp.at(mate_sub_vec_id); // "0+|..|..|.._1-|..|..|.."
				if(mate_reg_str2.compare("-")!=0){
					if(mate_reg_str2.find('_')==string::npos){ // single mate region
						mate_str_vec2 = split(mate_reg_str2, "|");
						mate_reg_id2 = stoi(mate_str_vec2.at(0).substr(0, 1));
						mate_orient_ch2 = mate_str_vec2.at(0).at(1);
						if(mate_reg_id2!=reg_id){
					...
					}else{ // two mate region
						flag = false;
						mate_str_vec2_tmp = split(mate_reg_str2, "_");
						for(const auto& item : mate_str_vec2_tmp){
							mate_str_vec2 = split(item, "|");
							mate_reg_id2 = stoi(mate_str_vec2.at(0).substr(0, 1));
							//mate_orient_ch2 = mate_str_vec2.at(0).at(1);
							if(mate_reg_id2==reg_id){ // found
								flag = true;
								break;
							}
						}
...
void checkSuppNum(mateClipReg_t &mate_clip_reg, int32_t support_num_thres){
	vector<string> mate_reg_str_vec, mate_str_vec, mate_str_vec_tmp;
	size_t i, j;
				...
				if(mate_clip_reg.bnd_mate_reg_strs[i].compare("-")!=0) {
					...
					for(j=0; j<2; j++){
						if(mate_reg_str_vec.at(j).compare("-")!=0){
							if(mate_reg_str_vec.at(j).find('_')==string::npos){ // single mate region
								...
								support_num = 0;
								mate_str_vec_tmp = split(mate_reg_str_vec.at(j), "_");
								for(const auto& item : mate_str_vec_tmp){
									mate_str_vec = split(item, "|");
									support_num_str = mate_str_vec.at(2);
									//cov_num_str = mate_str_vec.at(3);
									support_num += stoi(support_num_str);
									//cov_num = stoi(cov_num_str);
...
void setUltraLargeFlag(mateClipReg_t &mate_clip_reg, Paras *paras){
	...
	mate_clip_reg.dist_breakpoint = 0;
	mate_clip_reg.ultra_large_dist_flag = false;

	max_dist = INT_MIN;
	if(mate_clip_reg.valid_flag and (mate_clip_reg.leftClipReg or mate_clip_reg.leftClipReg2 or mate_clip_reg.rightClipReg or mate_clip_reg.rightClipReg2 or mate_clip_reg.large_indel_flag)){
		if(mate_clip_reg.large_indel_flag){ // large indel
			reg1 = mate_clip_reg.largeIndelClipReg;
			mate_clip_reg.dist_breakpoint = abs(reg1->sv_len);
			mate_clip_reg.ultra_large_dist_flag = false;
		}else{
			reg1 = mate_clip_reg.leftClipReg ? mate_clip_reg.leftClipReg : mate_clip_reg.leftClipReg2;
			reg2 = mate_clip_reg.rightClipReg ? mate_clip_reg.rightClipReg : mate_clip_reg.rightClipReg2;
			if(reg1 and reg2 and reg1->chrname.compare(reg2->chrname)==0){  // same chrome
				dist = abs(reg2->startRefPos - reg1->startRefPos);
				if(dist>max_dist) max_dist = dist;

				// left side
				if(mate_clip_reg.leftClipRegNum==2){
					dist =  abs(mate_clip_reg.leftClipReg2->startRefPos -  mate_clip_reg.leftClipReg->startRefPos);
					if(dist>max_dist) max_dist = dist;
				}

				// right side
				if(mate_clip_reg.rightClipRegNum==2){
					dist =  abs(mate_clip_reg.rightClipReg2->startRefPos -  mate_clip_reg.rightClipReg->startRefPos);
					if(dist>max_dist) max_dist = dist;
				}

				mate_clip_reg.dist_breakpoint = max_dist;
				if(mate_clip_reg.dist_breakpoint>paras->maxVarRegSize) mate_clip_reg.ultra_large_dist_flag = true;
			}
		}
	}
}
	...
	if(chr_len!=-1){
		...
		for(pos=start_pos; pos<=end_pos; pos++){
			...
			if(baseArray[pos-start_pos].coverage.idx_RefBase!=4){ // excluding 'N'
				//totalReadBeseNum += baseArray[pos-start_pos].coverage.num_bases[5]; // deleted on 2024-0718
				totalReadBeseNum += baseArray[pos-start_pos].coverage.num_bases[5] + baseArray[pos-start_pos].del_num_from_del_vec;
				totalRefBaseNum ++;
...
bool isSizeSatisfied(int64_t ref_dist, int64_t query_dist, int64_t min_sv_size_usr){
	...
	size_satisfy = false;
	if(dif>=min_sv_size_usr or ref_dist>=min_sv_size_usr or query_dist>=min_sv_size_usr)
		size_satisfy = true;
...
//	}
	...
	for(i=0;i<varVec.size();i++){
		if(abs(reg->sv_len)<abs(varVec[i]->sv_len)){
			dif_ratio = (double)abs(reg->sv_len)/abs(varVec[i]->sv_len);
		}else{
			dif_ratio = (double)abs(varVec[i]->sv_len)/abs(reg->sv_len);
		}
...
void processClipRegs(int32_t work_id, mateClipReg_t &mate_clip_reg, reg_t *clip_reg, vector<mateClipReg_t*> *mateClipRegVector, vector<reg_t*> *clipRegVector, vector<varCand*> &var_cand_clipReg_vec, vector<Block*> *blockVector, Paras *paras, pthread_mutex_t *p_mutex_mate_clip_reg){
	...
	if(mate_clip_reg.valid_flag){
		...
		if(mate_clip_reg.leftClipReg or mate_clip_reg.leftClipReg2 or mate_clip_reg.rightClipReg or mate_clip_reg.rightClipReg2 or mate_clip_reg.large_indel_flag){
			...
			clip_reg_new->dist_breakpoint = mate_clip_reg.dist_breakpoint;
			clip_reg_new->ultra_large_dist_flag = mate_clip_reg.ultra_large_dist_flag; 
		...
		if(mate_clip_reg.supp_num_valid_flag){
			...
			reg->merge_flag = clip_reg->merge_flag;
			reg->rescue_flag = clip_reg->rescue_flag;
			reg->gt_type = -1;
			reg->gt_seq = "";
			reg->AF = 0;
...
void *workProcessMonitor(void *arg){
	...
	int32_t uid, len, max_proc_running_minutes, est_total_minutes;
	double pcpu;
	vector<string> str_vec, tmp_vec, tmp_vec2;
	int32_t day, hour, minute, second, total_sec, total_min;
	pid_t pid;
	...
	while(1){
		if(isFileExist(work_finish_filename)){
			...
			uid = getuid();
			pscmd += " -o comm,pid,time,pcpu,uid | awk '$NF ~/" + to_string(uid) + "/'";
			//cout << pscmd << endl;
			...
			while(fgets(buffer, sizeof(buffer), fp)){
				...
				if(len>=5){
					...
					if(str_vec.at(len-3).find('-')==string::npos){
						tmp_vec = split(str_vec.at(len-3), ":");
						hour = stoi(tmp_vec.at(0));
						minute = stoi(tmp_vec.at(1));
						second = stoi(tmp_vec.at(2));
					}else{
						tmp_vec = split(str_vec.at(len-3), "-");
						...
						second = stoi(tmp_vec2.at(2));
					}
					total_min = day*24*60 + hour*60 + minute;
					total_sec = total_min * 60 + second;


					//compute cpu usage, added on 2026-03-08
					pcpu = stod(str_vec.at(len-2)) / 100.0;
					if(pcpu>0) est_total_minutes = (total_sec / pcpu) / 60.0;
					else est_total_minutes = total_min;

					//cout << "est_total_minutes=" << est_total_minutes << ", total_mins=" << total_min << endl;

					// kill process
					if(est_total_minutes>max_proc_running_minutes){
						pid = stoi(str_vec.at(len-4));
						kill(pid, SIGKILL);
...
// get total virtual size (in kB, 1024 bytes) of monitored processes
int64_t getTotalVirtSize(const string &proc_names){
	FILE *fp;
	char buffer[256] = {'\0'};
	string pscmd, line, vsz_str, str_tmp;
	int32_t len;
	vector<string> str_vec;
	int64_t total_vsz, vsz;

	pscmd = "ps -C " + proc_names;
	pscmd += " -o comm,vsz";
	//cout << pscmd << endl;
	fp = popen(pscmd.c_str(), "r");
	if(fp==NULL){
		cout << __func__ << ": popen error." << endl;
		exit(1);
	}
	...
	while(fgets(buffer, sizeof(buffer), fp)){
		...
		str_vec = split(line," ");
		len = str_vec.size();
		if(len>=2 and str_vec.at(0).compare("COMMAND")!=0){
			//get virtual size, added on 2026-03-09
			vsz_str = str_vec.at(len-1);
			if(vsz_str.at(vsz_str.size()-1)=='\n') vsz_str.erase(--vsz_str.end());
			vsz = stoi(vsz_str);
			total_vsz += vsz;
		}
	}
	pclose(fp);
	//cout << "........total_vsz=" << total_vsz << endl;
	...
	while(i<b->core.n_cigar or j<segs_MD.size()){
		...
		switch(op){
			...
			case BAM_CINS:
				// add alnSeg item
				ins_str = "";
				for(k=0; k<tmp_cigar_len; k++) ins_str += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];
	...
	while(i<b->core.n_cigar){
		switch(op){
			case BAM_CMATCH:
				...
				for(k=0; k<tmp_cigar_len; k++){
					if(startRpos+k>=startRefPos_paras and startRpos+k<=endRefPos_paras){
						queBase = "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];
			...
			case BAM_CDEL:
				// add alnSeg item
				del = "";
				for(k=0; k<tmp_cigar_len; k++) del += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];//something wrong?
			...
			case BAM_CDIFF:
				diff = "";
				for(k=0; k<tmp_cigar_len; k++) diff += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];
...
vector<struct alnSeg*> generateAlnSegs_no_MD2(bam1_t* b, Base* BaseArr, int64_t startRefPos_paras, int64_t endRefPos_paras){
	vector<struct alnSeg*> alnSegs;
	uint32_t *c, op, i = 0,startRpos, startQpos;
	int32_t k, tmp_cigar_len, mis_idx, startmatch_idx, endmatch_idx, tmp_sig_size;
	char queBase, refBase;
	string ins_str, diff, del;
	bool overlap_flag;
	int64_t end_ref_pos;
	uint8_t *seq_int;
		...
		switch(op){
			case BAM_CMATCH:
				if(overlap_flag){
					...
					for(k=0; k<tmp_cigar_len; k++){
						if(startRpos+k>=startRefPos_paras and startRpos+k<=endRefPos_paras){
							queBase = "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];
							refBase = BaseArr[startRpos+k-startRefPos_paras].coverage.refBase;
							if(isBaseMatch(queBase,refBase)==false){
			...
			case BAM_CDEL:
				if(overlap_flag){
					// add alnSeg item
					del = "";
					//for(k=0; k<tmp_cigar_len; k++) del += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];
//					dist1 = dist2 = 0;
//					if(startRpos<startRefPos_paras) dist1 = startRefPos_paras - startRpos;
//					if(startRpos+tmp_cigar_len-1>endRefPos_paras) dist2 = startRpos + tmp_cigar_len - 1 - endRefPos_paras;
					//del = refseq.substr(startRpos-startRefPos_paras+dist1, tmp_cigar_len-dist1-dist2);
					tmp_sig_size = 0;
					for(k=0; k<tmp_cigar_len; k++){
						if(startRpos+k>=startRefPos_paras and startRpos+k<=endRefPos_paras){
							refBase = BaseArr[startRpos+k-startRefPos_paras].coverage.refBase;
					...
					//alnSegs.push_back(allocateAlnSeg(startRpos, startQpos, tmp_cigar_len, BAM_CDEL, del));
					alnSegs.push_back(allocateAlnSeg(startRpos, startQpos, tmp_sig_size, BAM_CDEL, del));
				}
				startRpos += tmp_cigar_len;
				op = bam_cigar_op(c[++i]);
				tmp_cigar_len = bam_cigar_oplen(c[i]);
...
vector<struct alnSeg*> generateAlnSegs_no_MD2(bam1_t* b, string &refseq, int64_t startRefPos_paras, int64_t endRefPos_paras){
	vector<struct alnSeg*> alnSegs;
	uint32_t *c, op, i;
	int32_t k, tmp_cigar_len, mis_idx, startmatch_idx, endmatch_idx, dist1, dist2;
	char queBase, ref;
	string ins_str, diff, del;
	bool overlap_flag;
	int64_t startRpos, startQpos, end_ref_pos, leftHardClipSize;
	uint8_t *seq_int;
	...
	if(op==BAM_CHARD_CLIP) leftHardClipSize = tmp_cigar_len;
	else leftHardClipSize = 0;
		...
		switch(op){
			case BAM_CMATCH:
				if(overlap_flag){
					...
					for(k=0; k<tmp_cigar_len; k++){
						if(startRpos+k>=startRefPos_paras and startRpos+k<=endRefPos_paras){
							queBase = "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos-leftHardClipSize+k-1)];
							ref = refseq.at(startRpos+k-startRefPos_paras);
							if(isBaseMatch(queBase,ref)==false){
								if(startRpos+k>startRefPos_paras){
									if(mis_idx==-1) {
										...
										startmatch_idx = mis_idx + 1;
									endmatch_idx = k - 1;
									//alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx+1, startQpos+startmatch_idx, endmatch_idx-startmatch_idx, BAM_CMATCH, ""));
									alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx, startQpos+startmatch_idx, endmatch_idx-startmatch_idx+1, BAM_CEQUAL, ""));
								}
								//diff = "=ACMGRSVTWYHKDBN"[bam_seqi(bam_get_seq(b), startQpos+k-1)];
								diff = queBase;
					...
					if(mis_idx==-1){
						...
						if(startRpos+tmp_cigar_len-1>endRefPos_paras)
							endmatch_idx -= (startRpos+tmp_cigar_len-1) - endRefPos_paras;
						//alnSegs.push_back(allocateAlnSeg(startRpos, startQpos, tmp_cigar_len, BAM_CMATCH, ""));
						//alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx, startQpos+startmatch_idx, tmp_cigar_len-startmatch_idx, BAM_CEQUAL, ""));
						alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx, startQpos+startmatch_idx, endmatch_idx-startmatch_idx+1, BAM_CEQUAL, ""));
					}else {
						startmatch_idx = mis_idx + 1;
						...
						if(startRpos+tmp_cigar_len-1>endRefPos_paras)
							endmatch_idx -= (startRpos+tmp_cigar_len-1) - endRefPos_paras;
						//alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx+1, startQpos+startmatch_idx+1, tmp_cigar_len-startmatch_idx-1, BAM_CMATCH, ""));
						//alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx, startQpos+startmatch_idx, tmp_cigar_len-startmatch_idx, BAM_CEQUAL, ""));
						alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx, startQpos+startmatch_idx, endmatch_idx-startmatch_idx+1, BAM_CEQUAL, ""));
					}
				}
			...
			case BAM_CINS:
				if(overlap_flag){
					// add alnSeg item
					ins_str = "";
					for(k=0; k<tmp_cigar_len; k++) ins_str += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos-leftHardClipSize+k-1)];
					alnSegs.push_back(allocateAlnSeg(startRpos, startQpos, tmp_cigar_len, BAM_CINS, ins_str));
				}
			...
			case BAM_CDEL:
				if(overlap_flag){
					// add alnSeg item
					//del = "";
					//for(k=0; k<tmp_cigar_len; k++) del += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos-leftHardClipSize+k-1)];
					dist1 = dist2 = 0;
					if(startRpos<startRefPos_paras) dist1 = startRefPos_paras - startRpos;
					if(startRpos+tmp_cigar_len-1>endRefPos_paras) dist2 = startRpos + tmp_cigar_len - 1 - endRefPos_paras;
					del = refseq.substr(startRpos-startRefPos_paras+dist1, tmp_cigar_len-dist1-dist2);
					alnSegs.push_back(allocateAlnSeg(startRpos+dist1, startQpos, tmp_cigar_len-dist1-dist2, BAM_CDEL, del));
				}
			...
			case BAM_CDIFF:
				if(overlap_flag){
					...
					//for(k=0; k<tmp_cigar_len; k++)
//						if(startRpos+k>=startRefPos_paras and startRpos+k<=endRefPos_paras)
//							diff += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos+k-1)];
					//alnSegs.push_back(allocateAlnSeg(startRpos, startQpos, tmp_cigar_len, BAM_CDIFF, diff));

					for(k=startmatch_idx; k<=endmatch_idx; k++)
						diff += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, startQpos-leftHardClipSize+k-1)];
					alnSegs.push_back(allocateAlnSeg(startRpos+startmatch_idx, startQpos+startmatch_idx, endmatch_idx-startmatch_idx+1, BAM_CDIFF, diff));
				}
...
bool isQcSigMatch(qcSig_t *qc_sig, qcSig_t *seed_qc_sig, int64_t var_startRefPos, int64_t var_endRefPos, int64_t max_ref_dist_match, int32_t min_size_ignore_seqsim, double size_ratio_match_thres, double consist_ratio_match_thres, faidx_t *fai){
	bool match_flag = false, contain_flag;
	double size_ratio, seqsim_val, min_val;
	int64_t ref_distance, end_ref_pos, end_ref_pos_seed, var_startRefPos_tmp, var_endRefPos_tmp;
	vector<string> comp_seqs;

	if(qc_sig and seed_qc_sig){
		if(qc_sig->chrname.compare(seed_qc_sig->chrname)==0){
			// same cigar_op, ratio of signature size less than 0.7, and sequence seqsim >=0.8
			if(qc_sig->cigar_op==seed_qc_sig->cigar_op){
				if(qc_sig->cigar_op_len>0 and seed_qc_sig->cigar_op_len>0) {
					...
					if(size_ratio>=size_ratio_match_thres){
						var_startRefPos_tmp = var_startRefPos - max_ref_dist_match;
						var_endRefPos_tmp = var_endRefPos + max_ref_dist_match;
						if(var_startRefPos_tmp<1) var_startRefPos_tmp = 1;

						contain_flag = false;
						if(qc_sig->ref_pos>=var_startRefPos_tmp and qc_sig->ref_pos<=var_endRefPos_tmp and seed_qc_sig->ref_pos>=var_startRefPos_tmp and seed_qc_sig->ref_pos<=var_endRefPos_tmp)
							contain_flag = true;

						if(contain_flag and (qc_sig->cigar_op_len>min_size_ignore_seqsim or qc_sig->cigar_op_len>min_size_ignore_seqsim)){ // only consider large items
							match_flag = true;
						}else{
							if(qc_sig->cigar_op==BAM_CINS) ref_distance = abs(qc_sig->ref_pos - seed_qc_sig->ref_pos);
							else{
								end_ref_pos = getEndRefPosAlnSeg(qc_sig->ref_pos, qc_sig->cigar_op, qc_sig->cigar_op_len);
								end_ref_pos_seed = getEndRefPosAlnSeg(seed_qc_sig->ref_pos, seed_qc_sig->cigar_op, seed_qc_sig->cigar_op_len);
								if(isOverlappedPos(qc_sig->ref_pos, end_ref_pos, seed_qc_sig->ref_pos, end_ref_pos_seed)) ref_distance = 0;
								else ref_distance = abs(qc_sig->ref_pos - seed_qc_sig->ref_pos);
							}
							if(qc_sig->cigar_op==BAM_CINS){
								if(qc_sig->altseq.size()>0 and seed_qc_sig->altseq.size()>0 and (ref_distance<MAX_REF_DIST_SEQSIM or contain_flag)){
									if(qc_sig->altseq_complete_flag and seed_qc_sig->altseq_complete_flag){
										...
										if(ref_distance>0 and min_val/ref_distance > 1-consist_ratio_match_thres){
											...
											seqsim_val = computeVarseqSim(comp_seqs.at(0), comp_seqs.at(1));
										}else
											seqsim_val = computeVarseqSim(qc_sig->altseq, seed_qc_sig->altseq);
#if POA_ALIGN_DEBUG
										cout << "seqsim_val=" << seqsim_val << ", cigar_op_len1=" << qc_sig->cigar_op_len << ", cigar_op_len2=" << seed_qc_sig->cigar_op_len << ", qc_sig.altseq=" << qc_sig->altseq << ", seed_qc_sig.altseq=" << seed_qc_sig->altseq << endl;
#endif
										if((ref_distance<max_ref_dist_match or contain_flag) and seqsim_val>=consist_ratio_match_thres){
											match_flag = true;
										}
									}else{
										if(ref_distance<max_ref_dist_match or contain_flag){
											match_flag = true;
										}
									}
								}else{
...
qcSig_t *allocateQcSigNode(struct alnSeg *aln_seg){
	...
	qc_sig->reg_contain_flag = qc_sig->have_next_clip = qc_sig->merge_flag = false;
...
void destoryQuerySeqInfoAll(vector<struct querySeqInfoNode*> &query_seq_info_all){
	...
	for(size_t i=0; i<query_seq_info_all.size(); i++){
		...
		destroyQueryQcSig(q_node->qcSig_vec);
		delete q_node;
	}
...
void destroyQueryQcSig(vector<qcSig_t *> &qcSig_vec){
	qcSig_t *qcSig;
	for(size_t i=0; i<qcSig_vec.size(); i++){
		qcSig = qcSig_vec.at(i);
		delete qcSig;
	}
		...
		for(j=0; j<q_node->query_alnSegs.size(); j++){
			...
			switch (op) {
				case BAM_CMATCH:
					...
					break;
				case BAM_CINS:
					if(aln_seg->seglen<min_sv_size) seq.erase(query_startQpos - 1, aln_seg->seglen);
					else query_startQpos += aln_seg->seglen;
					break;
				case BAM_CDEL:
					if(aln_seg->seglen<min_sv_size){
						...
						seq.insert(query_startQpos - 1, aln_seg->seg_MD);
						query_startQpos += aln_seg->seglen;
					}
					break;
				case BAM_CSOFT_CLIP:
...
int32_t getReadGroupId(reg_t *reg, vector< vector<string> > &qnames_vec_cluster, varCand *var_cand){
	...
	group_id = -1;
	if(reg->query_id!=-1){
		if(reg->rescue_flag) tmp_cnsfilename = var_cand->rescue_cnsfilename;
	...
	//	}
		qname_vec = qnames_vec.at(reg->query_id);
		...
		for(i=0; i<qnames_vec_cluster.size(); i++){
			...
			for(j=0; j<qname_vec.size(); j++){
				...
				if(find(qname_vec_cluster.begin(), qname_vec_cluster.end(), qname) != qname_vec_cluster.end()){  // found
					check_num ++;
					if(check_num>=MAX_CHECK_READS_NUM or check_num==(int32_t)qname_vec.size()){
						group_id = i;
						break;
					}
				}
...
// 	int64_t current_read_end_pos;	

// 	startpos = -1;
// 	endpos = -1;

// 	// compute position
// 	current_read_end_pos = bam_endpos(b);
// 	if(startpos == -1 or b->core.pos < startpos) 
// 		startpos = b->core.pos;
// 	if(endpos == -1 || current_read_end_pos > endpos)
// 		endpos = current_read_end_pos;
...
double computeSigDensityFromAlnData(bam1_t *b, faidx_t *fai, bam_hdr_t *header, int64_t startRpos, int64_t endRpos){
	...
	double sig_density = 0;
	vector<struct alnSeg*> alnSegs;
	int32_t sig_num = 0;
	int32_t ref_start, ref_end, ref_span;

	char* chrname = header->target_name[b->core.tid];
	chrlen_tmp = faidx_seq_len(fai, chrname); // get the reference length
	if(chrlen_tmp!=-1){
		if(startRpos<1) startRpos = 1;
		if(endRpos>chrlen_tmp) endRpos = chrlen_tmp;

		reg_str = string(chrname) + ":" + to_string(startRpos) + "-" + to_string(endRpos);
		pthread_mutex_lock(&mutex_fai);
		seq = fai_fetch(fai, reg_str.c_str(), &seq_len);
		pthread_mutex_unlock(&mutex_fai);
		refseq = seq;
		...
		// cout << startRpos << "-" << endRpos << ":" << refseq.size() << endl;
		...
		if(!(b->core.flag & BAM_FUNMAP)){ // aligned
			...
			// sig_density = (double)sig_num/b->core.n_cigar;
			ref_start = b->core.pos; // left, 0-based
			ref_end = bam_endpos(b); // right, 0-based
			ref_span = ref_end - ref_start;
			// sig_density = 1000*(double)sig_num/b->core.l_qseq;
			sig_density = 1000*(double)sig_num/ref_span;

			// string qname = bam_get_qname(b);
			// cout << ref_start << "-" << ref_end << " " << qname << ":sig_num="<< sig_num << ", ref_span=" << ref_span << ", sig_density=" << sig_density << endl;
...
void rmNeighborFalseSig(vector<qcSig_t*> &sig_vec, int64_t max_ref_dist, double min_size_ratio){
	...
	for(i=1; i<sig_vec.size(); ){
		sig1 = sig_vec.at(i-1);
		sig2 = sig_vec.at(i);
		flag = false;
		dist = sig2->ref_pos - sig1->ref_pos;
		if(dist<max_ref_dist and ((sig1->cigar_op==BAM_CINS and sig2->cigar_op==BAM_CDEL) or (sig1->cigar_op==BAM_CDEL and sig2->cigar_op==BAM_CINS))){
			if(sig1->cigar_op_len<sig2->cigar_op_len) len_ratio = (double)sig1->cigar_op_len / sig2->cigar_op_len;
			else len_ratio = (double)sig2->cigar_op_len / sig1->cigar_op_len;
			if(len_ratio>=min_size_ratio){
				sig_vec.erase(sig_vec.begin()+i);
				sig_vec.erase(sig_vec.begin()+i-1);
				flag = true;
			}
		}
		if(flag==false) i++;
...
bool mergeNeighbouringSigsFlag(struct querySeqInfoNode* query_seq_info_node, vector<qcSig_t*> &sig_vec, int32_t min_ref_dist_thres, int32_t max_ref_dist_thres, double min_merge_seqsim_thres, double min_valid_sig_size_ratio_thres, faidx_t *fai){
	...
	if(query_seq_info_node->clip_aln and query_seq_info_node->clip_aln->bam){
			...
			for(j=i+1; j<sig_vec.size(); j++){
				...
				if(query_opflag == sig2->cigar_op and dist <= max_ref_dist_thres){
					...
					if(query_opflag==BAM_CDEL){ // DEL
						...
						if(query_seq_info_node->clip_aln->leftHardClippedFlag==false){
							start_querypos_comp = query2_startQpos - dist;
							end_querypos_comp = query2_startQpos - 1;
						}else{
							start_querypos_comp = query2_startQpos - query_seq_info_node->clip_aln->leftClipSize - dist;
							end_querypos_comp = query2_startQpos - query_seq_info_node->clip_aln->leftClipSize - 1;
						}
						if(start_querypos_comp<1) start_querypos_comp = 1;
						if(end_querypos_comp<1) end_querypos_comp = 1;
						...
						if(((double)dist/(sig1->cigar_op_len+sig2->cigar_op_len+dist)<min_valid_sig_size_ratio_thres and dist<min_ref_dist_thres) or sim_val>=min_merge_seqsim_thres){
							flag = true;
							sig1->cigar_op_len += query2_svlen;
							sig1->merge_flag = true;
						...
						if(query_seq_info_node->clip_aln->leftHardClippedFlag==false){
							...
							start_querypos_comp = query2_endQpos - query_seq_info_node->clip_aln->leftClipSize - dist;
							end_querypos_comp = query2_endQpos - query_seq_info_node->clip_aln->leftClipSize;
						}
						if(start_querypos_comp<1) start_querypos_comp = 1;
						if(end_querypos_comp<1) end_querypos_comp = 1;
						...
						//end_refpos_comp = query2_startRpos - 1; // deleted on 2026-03-22
						end_refpos_comp = query2_startRpos;
						// get compared ref seq
						reg_str = query_seq_info_node->clip_aln->chrname + ":" + to_string(start_refpos_comp) + "-" + to_string(end_refpos_comp);
						pthread_mutex_lock(&mutex_fai);
						seq = fai_fetch(fai, reg_str.c_str(), &seq_len);
						...
						if(((double)dist/(sig1->cigar_op_len+sig2->cigar_op_len+dist)<min_valid_sig_size_ratio_thres and dist < min_ref_dist_thres) or sim_val >= min_merge_seqsim_thres){
							...
							if(query_seq_info_node->clip_aln->leftHardClippedFlag==false){
								...
								start_querypos_merged = query1_endQpos - query_seq_info_node->clip_aln->leftClipSize;
								end_querypos_merged = query1_endQpos - query_seq_info_node->clip_aln->leftClipSize + query2_svlen;
							}
							if(start_querypos_comp<1) start_querypos_comp = 1;
							if(end_querypos_comp<1) end_querypos_comp = 1;
		...
		// 		comp_queryseq = "";
		// 		for(l=start_querypos_comp-1; l<end_querypos_comp; l++) comp_queryseq += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, l)];  // seq
		// 		sim_val = computeVarseqSim(comp_queryseq, sig_vec.at(i)->altseq);
		// 		cout << "sim_val=" << sim_val << ", comp_queryseq=" << comp_queryseq << ", sig_vec.at("<< i << ")->altseq=" << sig_vec.at(i)->altseq << endl;
		// 	}
		// }
...
// merge neighbouring signatures for clipping regions
...
//	if(qcSigList_node->query_seqs_vec.at(0)->qname.compare("SRR18363754.906769")==0)
//		cout << "--------------qname=" << qcSigList_node->query_seqs_vec.at(0)->qname << "--------------" << endl;

	queryseq_whole = "";
	for(i=0; i<qcSigList_node->query_seqs_vec.size(); i++){
		queryseq_node = qcSigList_node->query_seqs_vec.at(i);
	...
	dist = -1;
	flag = false;
//	if(queryseq_node->clip_aln and queryseq_node->clip_aln->bam){
	if(queryseq_node->clip_aln){
//		seq_int = bam_get_seq(queryseq_node->clip_aln->bam);
		for(i=0; i<sig_vec.size(); i++){
			sig1 = sig_vec.at(i);
			// skip normal signature
			if(sig1->cigar_op!=BAM_CINS and sig1->cigar_op!=BAM_CDEL) continue;
			...
			for(j=i+1; j<sig_vec.size(); j++){
				...
				if(query_opflag == sig2->cigar_op and dist <= max_ref_dist_thres){
					...
					if(query_opflag==BAM_CDEL){ // DEL
						...
						// get compared query seq
						comp_queryseq = "";
//						for(l=start_querypos_comp-1; l<end_querypos_comp; l++) comp_queryseq += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, l)];  // seq, deleted on 2026-03-20
						if(start_querypos_comp<=(int64_t)queryseq_whole.size() and start_querypos_comp<=end_querypos_comp)
							comp_queryseq = queryseq_whole.substr(start_querypos_comp-1, end_querypos_comp-start_querypos_comp+1);
//						cout << "line=" << __LINE__ << endl;
//						cout << "\tcomp_queryseq =" << comp_queryseq << endl;
//						cout << "\tcomp_queryseq2=" << comp_queryseq2 << endl;
						...
						start_refpos_comp = query2_endRpos - dist;
						end_refpos_comp = query2_endRpos - 1;
						if(end_refpos_comp<start_refpos_comp) end_refpos_comp = start_refpos_comp;

						// get compared ref seq
						reg_str = queryseq_node->clip_aln->chrname + ":" + to_string(start_refpos_comp) + "-" + to_string(end_refpos_comp);
						pthread_mutex_lock(&mutex_fai);
						seq = fai_fetch(fai, reg_str.c_str(), &seq_len);
						pthread_mutex_unlock(&mutex_fai);
						comp_refseq = seq;
						...
						//cout << "sig1_op_len=" << sig1->cigar_op_len << "_" << sig1->cigar_op << ", sig2_op_len=" << sig2->cigar_op_len << "_" << sig2->cigar_op << endl;
						//cout << "comp_queryseq.size=" << comp_queryseq.size() << ", comp_refseq.size=" << comp_refseq.size() << endl;
						sim_val = computeVarseqSim(comp_queryseq, comp_refseq);
						// cout << "sim_val=" << sim_val << endl;
						...
						if(((double)dist/(sig1->cigar_op_len+sig2->cigar_op_len+dist)<min_valid_sig_size_ratio_thres and dist<min_ref_dist_thres) or sim_val>=min_merge_seqsim_thres){
							flag = true;
							//sig1->cigar_op_len += query2_svlen;
							sig1->cigar_op_len += sig2->cigar_op_len;
							sig1->merge_flag = true;
						...
						// get compared query seq
						comp_queryseq = "";
//						for(l=start_querypos_comp; l<end_querypos_comp; l++) comp_queryseq += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, l)];  // seq, deleted on 2026-03-20
						if(start_querypos_comp<=(int64_t)queryseq_whole.size() and start_querypos_comp<=end_querypos_comp)
							comp_queryseq = queryseq_whole.substr(start_querypos_comp-1, end_querypos_comp-start_querypos_comp+1);
//						cout << "line=" << __LINE__ << endl;
//						cout << "\tcomp_queryseq =" << comp_queryseq << endl;
						...
						//end_refpos_comp = query2_startRpos - 1; // deleted on 2026-03-22
						end_refpos_comp = query2_startRpos;
						if(start_refpos_comp<1) start_refpos_comp = 1;
						if(end_refpos_comp<start_refpos_comp) end_refpos_comp = start_refpos_comp;
						// get compared ref seq
						reg_str = queryseq_node->clip_aln->chrname + ":" + to_string(start_refpos_comp) + "-" + to_string(end_refpos_comp);
						pthread_mutex_lock(&mutex_fai);
						...
						if(((double)dist/(sig1->cigar_op_len+sig2->cigar_op_len+dist)<min_valid_sig_size_ratio_thres and dist < min_ref_dist_thres) or sim_val >= min_merge_seqsim_thres){
							...
							start_querypos_merged = query1_endQpos;
							end_querypos_merged = query1_endQpos + query2_svlen;
							if(end_querypos_merged>(int64_t)queryseq_whole.size()) end_querypos_merged = queryseq_whole.size();
							merging_seq = "";
//							for(l=start_querypos_merged-1; l<end_querypos_merged; l++) merging_seq += "=ACMGRSVTWYHKDBN"[bam_seqi(seq_int, l)]; // deleted on 2026-03-20
							if(start_querypos_merged<=(int64_t)queryseq_whole.size() and start_querypos_merged<=end_querypos_merged)
								merging_seq = queryseq_whole.substr(start_querypos_merged-1, end_querypos_merged-start_querypos_merged+1);
//							cout << "line=" << __LINE__ << endl;
//							cout << "\merging_seq =" << merging_seq << endl;
...
bool desc_cmp_size_vec(const pair<int32_t, vector<int32_t>> &a, const pair<int32_t, vector<int32_t>> &b){
	bool flag;
	if(a.second.size()>b.second.size()) flag = true;
	else if(a.second.size()<b.second.size()) flag = false;
	else{ // equal
		if(abs(a.first)>abs(b.first)) flag = true;
		else flag = false;
	}
	return flag;
}
...
//			query_pos1 = clip_aln1->endRefPos;
			//query_pos1 = clip_aln1->querylen - clip_aln1->endQueryPos + 1;
			query_pos1 = clip_aln1->endQueryPos;
...
//				query_pos2 = clip_aln2->endRefPos;
				//query_pos2 = clip_aln2->querylen - clip_aln2->endQueryPos + 1;
				query_pos2 = clip_aln2->endQueryPos;
...
bool isSameOrient(vector<clipAlnData_t*> &query_aln_segs, int32_t min_aln_size){
	...
	i = 0;
	while(i<query_aln_segs.size()){
		// find next segment idx
		next_id = -1;
		for(j=i+1; j<query_aln_segs.size(); j++){
			if(query_aln_segs.at(i)->aln_orient!=query_aln_segs.at(j)->aln_orient or query_aln_segs.at(j)->ref_dist>=min_aln_size or query_aln_segs.at(j)->query_dist>=min_aln_size){
				next_id = j;
				break;
			}
		}
		...
		if(next_id!=-1){
			if(query_aln_segs.at(i)->aln_orient!=query_aln_segs.at(next_id)->aln_orient){
				flag = false;
				break;
			}
			i = next_id;
...
bool isSameAlnReg(vector<clipAlnData_t*> &query_aln_segs, int64_t maxVarRegSize){
	...
	clipAlnData_t *aln_seg1, *aln_seg2;
		...
		if(same_aln_reg_flag){
			...
			//if(same_orient_flag){ // same orient
				for(i=1; i<query_aln_segs.size(); i++){
					aln_seg1 = query_aln_segs.at(i-1);
					aln_seg2 = query_aln_segs.at(i);
					adjacent_flag = isAdjacentClipAlnSeg(aln_seg1, aln_seg2, maxVarRegSize);
...
bool isTurnRound(vector<clipAlnData_t*> &query_aln_segs, int64_t leftClipRefPos, int64_t rightClipRefPos){
	...
	for(i=0; i<query_aln_segs.size(); i++){
		clip_aln = query_aln_segs.at(i);
		if(left_aln_seg==NULL and clip_aln->leftmost_flag) left_aln_seg = clip_aln;
		if(right_aln_seg==NULL and clip_aln->rightmost_flag) right_aln_seg = clip_aln;
	}

	if(left_aln_seg==NULL or right_aln_seg==NULL){ // get the minimum and maximum
		min_idx = max_idx = -1;
		min_qpos = INT_MAX, max_qpos = INT_MIN;
		for(i=0; i<query_aln_segs.size(); i++){
			clip_aln = query_aln_segs.at(i);
			if(clip_aln->startQueryPos<clip_aln->endQueryPos){
				tmp_min = clip_aln->startQueryPos;
				tmp_max = clip_aln->endQueryPos;
			}else{
				tmp_min = clip_aln->endQueryPos;
				tmp_max = clip_aln->startQueryPos;
			}
			if(tmp_min<min_qpos){
				min_idx = i;
				min_qpos = tmp_min;
		...
		if(left_aln_seg==NULL) { query_aln_segs.at(min_idx)->leftmost_flag = true; left_aln_seg = clip_aln; }
		if(right_aln_seg==NULL) { query_aln_segs.at(max_idx)->rightmost_flag = true; right_aln_seg = clip_aln; }
	}

	flag = false;
	if(clip_aln){
		clip_aln = left_aln_seg;
		if(clip_aln->aln_orient==ALN_PLUS_ORIENT) clip_end1 = RIGHT_END;
		else clip_end1 = LEFT_END;
		clip_end2 = -1;
		//cout << "enter " << clip_aln->queryname << endl;
		while(clip_aln and clip_aln->rightmost_flag==false){
			...
			if(clip_aln_next){
				if(clip_end1==RIGHT_END){
					if(clip_aln->aln_orient==clip_aln_next->aln_orient) clip_end2 = RIGHT_END;
					else clip_end2 = LEFT_END;
				}else{
					if(clip_aln->aln_orient==clip_aln_next->aln_orient) clip_end2 = LEFT_END;
					else clip_end2 = RIGHT_END;
				}
			}
			clip_aln = clip_aln_next;
			clip_end1 = clip_end2;
		}
	}
	//cout << "flag=" << flag << ", exiting ..." << endl;
	return flag;
}
...
int32_t getOccNumInVecs(vector<clipAlnData_t*> &query_aln_segs, vector<clipPos_t*> &leftClipPosVector, vector<clipPos_t*> &leftClipPosVector2, vector<clipPos_t*> &rightClipPosVector, vector<clipPos_t*> &rightClipPosVector2){
	...
	for(i=0; i<query_aln_segs.size(); i++){
		clip_aln = query_aln_segs.at(i);
		// check leftVec1
		for(j=0; j<leftClipPosVector.size(); j++){
			clip_pos = leftClipPosVector.at(j);
			if(clip_pos->clip_aln and clip_pos->clip_aln==clip_aln) {
				occ_num ++;
				break;
			}
		}
		// check leftVec2
		for(j=0; j<leftClipPosVector2.size(); j++){
			clip_pos = leftClipPosVector2.at(j);
			if(clip_pos->clip_aln and clip_pos->clip_aln==clip_aln) {
				occ_num ++;
				break;
			}
		...
		// check rightVec1
		for(j=0; j<rightClipPosVector.size(); j++){
			clip_pos = rightClipPosVector.at(j);
			if(clip_pos->clip_aln and clip_pos->clip_aln==clip_aln) {
				occ_num ++;
				break;
			}
		...
		// check rightVec2
		for(j=0; j<rightClipPosVector2.size(); j++){
			clip_pos = rightClipPosVector2.at(j);
			if(clip_pos->clip_aln and clip_pos->clip_aln==clip_aln) {
				occ_num ++;
				break;
			}
