	...
	if(clipAlnDataVector.size()>0){
		...
		if(clip_reg_flag){ // clipping region
			if(query_seq_info_all.size()>0){
				...
				clipRegCluster clip_reg_cluster(minClipEndSize, min_sv_size);
...
vector<int8_t> LocalCns::computeQcMatchProfileSingleQuery(queryCluSig_t *queryCluSig, queryCluSig_t *seed_qcQuery){
	...
	if(queryCluSig->qcSig_vec.size()>0 or seed_qcQuery->qcSig_vec.size()>0){
		...
		for(i=1; i<rowsNum; i++){
			for(j=1; j<colsNum; j++){
				matchFlag = isQcSigMatch(queryCluSig->qcSig_vec.at(i-1), seed_qcQuery->qcSig_vec.at(j-1), QC_SIZE_RATIO_MATCH_THRES_INDEL);
...
//bool LocalCns::isQcSigMatch(qcSig_t *qc_sig, qcSig_t *seed_qc_sig, double size_ratio_match_thres){
...
void LocalCns::recordCnsInfo(ofstream &cns_info_file){
	...
	line = refseqfilename + "\t" + contigfilename + "\t" + readsfilename + "\t" + left_shift_size_str + "\t" + right_shift_size_str + "\t" + cns_status;
