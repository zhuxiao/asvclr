...
#define CNS_MIN_SEQSIM_CLUSTER_THRES				(0.8f)
...
public:
	indelRegCluster(string &chrname, int64_t var_startRefPos, int64_t var_endRefPos, int32_t sv_len_est, int64_t startRefPos_cns, int64_t endRefPos_cns, int32_t min_sv_size, int32_t min_supp_num, double min_seqsim_match, faidx_t *fai);
...
private:
	...
	double computeMatchRatio(struct querySeqInfoNode *query_seq_info_node, struct querySeqInfoNode *q_cluster_node, int64_t startSpanPos, int64_t endSpanPos, int32_t min_sv_size, double min_seqsim_match);
	...
	vector<int8_t> computeQcMatchProfileSingleQuery(queryCluSig_t *queryCluSig, queryCluSig_t *seed_qcQuery, bool mergeFlag, double min_seqsim_match);
