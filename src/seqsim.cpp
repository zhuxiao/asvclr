...
double calculate_seqsim(const string& seq1, const string& seq2) {
    ...
    double seqsim = static_cast<double>(matching_chars + relief) / seq_length;
    return seqsim;
}
...
double computeVarseqSim(const string& seq, const string& seq1) {
    string  seq1_new, seq2_new, seq_tmp, seq1_tmp, AlignSeq, AlignSeq1;
    double result;
...
//      needleman_wunschOp(seq, seq1, MATCHSCORE, MISMATCHSCORE, GAPPENALTY, seq1_new, seq2_new);
    	needleman_wunschOp(seq_tmp, seq1_tmp, MATCHSCORE, MISMATCHSCORE, GAPPENALTY, seq1_new, seq2_new);
        result = calculate_seqsim(seq1_new, seq2_new);
    }else{
...
//        vector<Minimizer> minimizers1 = findMinimizers(seq1, WINDOWSIZE, KMERSIZE);
        vector<Minimizer> minimizers = findMinimizers(seq_tmp, WINDOWSIZE, KMERSIZE);
        vector<Minimizer> minimizers1 = findMinimizers(seq1_tmp, WINDOWSIZE, KMERSIZE);
...
//        LongSequenceSplitAlignment(minimizers, minimizers1, KMERSIZE, AlignSeq, AlignSeq1, seq, seq1);
        LongSequenceSplitAlignment(minimizers, minimizers1, KMERSIZE, AlignSeq, AlignSeq1, seq_tmp, seq1_tmp);
...
//            needleman_wunschOp(seq, seq1, MATCHSCORE, MISMATCHSCORE, GAPPENALTY, seq1_new, seq2_new);
            ...
            result = calculate_seqsim(AlignSeq, AlignSeq1);
    }
    return result;
}
