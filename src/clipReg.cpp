...
void clipReg::fillClipAlnDataVectorWithSATag(){
	clipAlnDataLoader clip_aln_data_loader(chrname, startRefPos, endRefPos, inBamFile, minClipEndSize, paras->maxVarRegSize, paras->minMapQ, paras->minHighMapQ);
...
void clipReg::extractClipPosVec(){
	...
	vector<clipPos_t*> clippos_vec_indel;
...
//			if(queryname.compare("SRR8858454.1.141483")==0){
//				cout << queryname << endl;
//			}
			...
			for(j=0; j<query_aln_segs.size(); j++){
				...
				if(clip_aln_seg->MapQ<paras->minMapQ and (clip_aln_seg->query_dist<MIN_ALN_SEG_SIZE_CLIPREG or clip_aln_seg->ref_dist<MIN_ALN_SEG_SIZE_CLIPREG) and (double)clip_aln_seg->query_dist/clip_aln_seg->querylen<LONGER_NON_SA_SEG_RATIO_THRES)
					continue;
				...
				if(valid_query_end_flag){
					...
					skip_flag = false;
					adjClipAlnSegInfo = getAdjacentClipAlnSeg(j, clip_end_flag, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
					...
					if(mate_arr_idx!=-1){ // mated
						if(mate_arr_idx_same_chr!=-1){
							...
							if((mate_clip_end_flag_same_chr==LEFT_END and mate_clip_aln_seg->left_aln) or (mate_clip_end_flag_same_chr==RIGHT_END and mate_clip_aln_seg->right_aln)){
								...
								//if(abs(min_ref_dist_same_chr)<=MAX_REF_DIST_SAME_CHR){ // different chromosomes with near reference distance
									...
									if(abs(min_dist_same_chr)>=MAX_REF_DIST_SAME_CHR and abs(min_dist_same_chr)<=paras->maxVarRegSize and abs(min_ref_dist_same_chr)>MAX_REF_DIST_SAME_CHR and abs(min_ref_dist_same_chr)<=paras->maxVarRegSize){
						...
						if((mate_clip_end_flag==LEFT_END and mate_clip_aln_seg->left_clip_checked_flag==false and mate_clip_aln_seg->chrname.compare(chrname)==0) or (mate_clip_end_flag==RIGHT_END and mate_clip_aln_seg->right_clip_checked_flag==false and mate_clip_aln_seg->chrname.compare(chrname)==0)){ // same chromosome
							...
							if(mate_clip_end_flag==LEFT_END){
								...
								if(mate_clip_pos_item->aln_orient==clip_pos_item->aln_orient) mate_clip_vec_idx = 0;
								else mate_clip_vec_idx = 1;
							}else{
								...
								if(mate_clip_pos_item->aln_orient==clip_pos_item->aln_orient) mate_clip_vec_idx = 1;
								else mate_clip_vec_idx = 0;
							}
						...
						}else if((mate_clip_end_flag==LEFT_END and mate_clip_aln_seg->left_clip_checked_flag==false and mate_clip_aln_seg->chrname.compare(chrname)!=0) or (mate_clip_end_flag==RIGHT_END and mate_clip_aln_seg->right_clip_checked_flag==false and mate_clip_aln_seg->chrname.compare(chrname)!=0)) { // different chromosomes, added on 2025-09-10
							mate_clip_pos_item = new clipPos_t();
							mate_clip_pos_item->chrname = mate_clip_aln_seg->chrname;
							mate_clip_pos_item->clip_aln = mate_clip_aln_seg;
							mate_clip_pos_item->clip_end = mate_clip_end_flag;
							...
							if(clip_vec_idx==mate_clip_vec_idx){ // deal with same part
								...
								if(mate_clip_vec_idx_tmp!=-1){
									...
									valid_mate_query_end_flag = false;
							//}else if(clip_pos_item->chrname.compare(mate_clip_pos_item->chrname)!=0 or clip_pos_item->clipRefPos<=mate_clip_pos_item->clipRefPos){
							}else if(clip_pos_item->clipRefPos<=mate_clip_pos_item->clipRefPos){
								clip_vec_idx = 0;
								mate_clip_vec_idx = 2;
							}else{
								clip_vec_idx = 1;
							...
							clip_pos_item->self_overlap_flag = mate_clip_pos_item->self_overlap_flag = false;
						}
					}
...
//						if((same_orient_flag==false and dist>10000)or((clip_pos_item->chrname.compare(mate_clip_pos_item->chrname)!=0)and same_orient_flag==false)) valid_mate_query_end_flag = false;
					...
					// save to indel vector
					if(skip_flag){
						clippos_vec_indel.push_back(clip_pos_item);
						clippos_vec_indel.push_back(mate_clip_pos_item);
					}
	...
	if(seg_skip_num>=paras->minReadsNumSupportSV or ref_skip_num>=paras->minReadsNumSupportSV){
		removeOtherClips(leftClipPosVector, clippos_vec_indel);
		removeOtherClips(leftClipPosVector2, clippos_vec_indel);
		removeOtherClips(rightClipPosVector, clippos_vec_indel);
		removeOtherClips(rightClipPosVector2, clippos_vec_indel);
	}
...
//					right_aln = getAdjacentClipAlnSegPreferSameChr(clip_pos->clip_aln, clip_pos->clip_end, query_aln_segs, minClipEndSize, paras->maxVarRegSize);
//					if(right_aln==NULL) right_aln = clip_pos->clip_aln->right_aln;

					seg_skip_flag = false;
					idx_vec = getVecIdxClipAlnData(clip_pos->clip_aln, query_aln_segs);
					adjClipAlnSegInfo = getAdjacentClipAlnSeg(idx_vec, clip_pos->clip_end, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
					mate_arr_idx = adjClipAlnSegInfo.at(0);
					...
					if(inner_missing_valid_flag and right_aln and clip_pos->clip_aln->rightClipSize>=minClipEndSize and ((right_aln->aln_orient==clip_pos->clip_aln->aln_orient and right_aln->ref_dist>=minAlnSize_same_orient)
						...
						if(clip_pos->clip_aln->aln_orient==right_aln->aln_orient){ // same orient
							...
							right_aln2 = getAdjacentClipAlnSegPreferSameChr(right_aln, RIGHT_END, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
	...
	//									}
							...
							left_aln2 = getAdjacentClipAlnSegPreferSameChr(right_aln, LEFT_END, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
	...
	//									}
				...
				if(clip_pos->clip_aln and ((clip_pos->same_orient_flag and clip_pos->clip_aln->ref_dist>=minAlnSize_same_orient) or (clip_pos->same_orient_flag==false and clip_pos->clip_aln->ref_dist>=minAlnSize_diff_orient))){
//					left_aln = getAdjacentClipAlnSegPreferSameChr(clip_pos->clip_aln, clip_pos->clip_end, query_aln_segs, minClipEndSize, paras->maxVarRegSize);
//					if(left_aln==NULL) left_aln = clip_pos->clip_aln->left_aln;
					...
					if(inner_missing_valid_flag and left_aln and clip_pos->clip_aln->leftClipSize>=minClipEndSize and ((left_aln->aln_orient==clip_pos->clip_aln->aln_orient and left_aln->ref_dist>=minAlnSize_same_orient)
						...
						if(clip_pos->clip_aln->aln_orient==left_aln->aln_orient){ // same orient
							left_aln2 = getAdjacentClipAlnSegPreferSameChr(left_aln, LEFT_END, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
	...
	//									}
							...
							right_aln2 = getAdjacentClipAlnSegPreferSameChr(left_aln, RIGHT_END, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
	...
	//									}
						...
						if(clip_aln_mate_flag and clip_pos->clip_aln->rightClipSize>=min_seg_size){
							...
							right_aln = getAdjacentClipAlnSegPreferSameChr(clip_pos->clip_aln, RIGHT_END, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
							if(right_aln==NULL) right_aln = clip_pos->clip_aln->right_aln;
	...
	if(adj_aln){ // multiple segments
		for(i=0; i<clip_pos_vec.size(); i++){
			...
			if(clip_pos_tmp->clip_aln!=clip_aln and clip_pos_tmp->chrname.compare(clip_aln->chrname)==0 and ((clip_pos_tmp->clip_end==clip_end and abs(clip_pos_tmp->clipRefPos-clip_loc)<=GROUP_DIST_THRES) or (clip_pos_tmp->clip_end==adj_clip_end and abs(clip_pos_tmp->clipRefPos-adj_clip_loc)<=GROUP_DIST_THRES))){
				...
				adjClipAlnSegInfo = getAdjacentClipAlnSeg(idx_vec, clip_pos_tmp->clip_end, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
				mate_arr_idx = adjClipAlnSegInfo.at(0);
				mate_clip_end = adjClipAlnSegInfo.at(1);
				mate_arr_idx_same_chr = adjClipAlnSegInfo.at(4);
	//			mate_clip_end_same_chr = adjClipAlnSegInfo.at(5);
...
// remove other clips
void clipReg::removeOtherClips(vector<clipPos_t*> &clipPosVector, vector<clipPos_t*> &clippos_vec_indel){
	size_t i;
	clipPos_t *clip_pos;

	for(i=0; i< clipPosVector.size(); ){
		clip_pos = clipPosVector.at(i);
		if(find(clippos_vec_indel.begin(), clippos_vec_indel.end(), clip_pos)==clippos_vec_indel.end()){ // not found, then delete
			delete clip_pos;
...
void clipReg::AdjustClipPosVecByBPConsistencySingleVec(vector<clipPos_t*> &clip_pos_vec, vector<clipPos_t*> &clip_pos_vec_main){
	...
	if(clip_pos_vec_main.size()>0){
		for(size_t i=0; i<clip_pos_vec.size(); ){
			...
			adjClipAlnSegInfo = getAdjacentClipAlnSeg(idx_vec, clip_pos_item->clip_end, query_aln_segs, minClipEndSize, paras->maxVarRegSize, paras->minMapQ);
			mate_arr_idx = adjClipAlnSegInfo.at(0);
			mate_clip_end = adjClipAlnSegInfo.at(1);
			//mate_arr_idx_same_chr = adjClipAlnSegInfo.at(4);
			//mate_clip_end_same_chr = adjClipAlnSegInfo.at(5);
...
void clipReg::appendClipPosSingleVec(vector<clipPos_t*> &clipPosVec, vector<clipAlnData_t*> &clipAlnDataVec){
	...
	if(clipPosVec.size()){
		...
		if(clip_pos_str_vec.size()){
			...
			chr_len = faidx_seq_len64(fai, chrname_max.c_str()); // get the reference length
			if(chr_len!=-1){
				if(start_pos<1) start_pos = 1;
				if(end_pos>chr_len) end_pos = chr_len;

				// load the clipping data
				clipAlnDataLoader clip_aln_data_loader(chrname_max, start_pos, end_pos, inBamFile, minClipEndSize, paras->maxVarRegSize, paras->minMapQ, paras->minHighMapQ);
				clip_aln_data_loader.loadClipAlnDataWithSATag(clipAlnDataVec, paras->max_ultra_high_cov);
				//cout << "line=" << __LINE__ << ", " << chrname_max << ":" << start_pos << "-" << end_pos << ", clipAlnDataVec.size=" << clipAlnDataVec.size() << endl;
...
void clipReg::removeFakeClips(){
	...
	removeFakeClipsLowCov(leftClipPosVector, paras->minReadsNumSupportSV*READS_NUM_SUPPORT_FACTOR);
	removeFakeClipsLowCov(leftClipPosVector2, paras->minReadsNumSupportSV*READS_NUM_SUPPORT_FACTOR);
	removeFakeClipsLowCov(rightClipPosVector, paras->minReadsNumSupportSV*READS_NUM_SUPPORT_FACTOR);
	removeFakeClipsLowCov(rightClipPosVector2, paras->minReadsNumSupportSV*READS_NUM_SUPPORT_FACTOR);

	// remove fake items of low ratio, added on 2026-04-13
	removeFakeClipsLowCovRatio(leftClipPosVector, leftClipPosVector2, paras->minReadsNumSupportSV, MIN_VALID_SIG_SIZE_RATIO_THRES);
	removeFakeClipsLowCovRatio(rightClipPosVector, rightClipPosVector2, paras->minReadsNumSupportSV, MIN_VALID_SIG_SIZE_RATIO_THRES);
...
void clipReg::removeFakeClipsDifferentChrSingleVec(vector<clipPos_t*> &clipPosVector){
	...
	for(i=0; i<clipPosVector.size(); i++)
		if(find(chrname_vec.begin(), chrname_vec.end(), clipPosVector.at(i)->chrname)==chrname_vec.end()) // not exist in vector, add it to vector
...
// remove fake clips with long dist based on single vector
void clipReg::removeFakeClipsLowCov(vector<clipPos_t*> &clipPosVector, double min_clip_reads_num){
	if(clipPosVector.size()<min_clip_reads_num) destroyClipPosVec(clipPosVector); // remove fake clips Only this function modifies the minimum number of supports
}

// remove fake clips with long dist based on single vector
void clipReg::removeFakeClipsLowCovRatio(vector<clipPos_t*> &clipPosVector, vector<clipPos_t*> &clipPosVector2, double min_clip_reads_num, double min_ratio_thres){
	int32_t min_val, vec_size1, vec_size2;
	double ratio;

	if(clipPosVector.size()>0 and clipPosVector2.size()>0){
		vec_size1 = clipPosVector.size();
		vec_size2 = clipPosVector2.size();

		ratio = -1;
		min_val = -1;
		if(vec_size1>vec_size2) { min_val = vec_size2; ratio = (double)vec_size2 / vec_size1; }
		else { min_val = vec_size1; ratio = (double)vec_size1 / vec_size2; }
		if(ratio!=-1 and ratio<MIN_VALID_SIG_SIZE_RATIO_THRES and min_val>0 and min_val<paras->minReadsNumSupportSV){
			if(vec_size1>vec_size2) destroyClipPosVec(clipPosVector2);
			else destroyClipPosVec(clipPosVector);
		}
	}
}
...
string clipReg::computeMateSingleRegStrForBND(vector<clipPos_t*> &clip_pos_vec, int32_t vec_id, string &chrname_clip, int64_t meanClipPos){
	string mate_reg_str, left_mate_str, right_mate_str, mate_str2, vec_id_str, orient_str, mate_clip_pos_str; //, bnd_str;
	size_t i;
	clipPos_t *clip_pos;
	clipAlnData_t *clip_aln, *clip_aln2;
	int32_t j, clip_end2, vec_id2, num_arr[4], orient_num_arr[2][4];
	int32_t maxValue, maxIdx, secValue, secIdx, support_num, cov_num, mate_reg_num;
...
//	cout << num_arr[3] << endl;
	...
	maxValue = maxIdx = secValue = secIdx = -1;
	for(j=0; j<4; j++){
		if(num_arr[j]>0) {
			if(num_arr[j]>maxValue){
				secValue = maxValue;
				secIdx = maxIdx;
				maxValue = num_arr[j];
				maxIdx = j;
			}else if(num_arr[j]>secValue){
				secValue = num_arr[j];
				secIdx = j;
			}
		}
	}
	//cout << "maxIdx=" << maxIdx << ", maxValue=" << maxValue << ", secIdx=" << secIdx << ", secValue=" << secValue << ", support_num=" << support_num << endl;
	...
	if(maxIdx!=-1 and maxValue>=MIN_CLIP_REG_MATED_RATIO*support_num){
		...
		mate_reg_num = 1;
		if(secIdx!=-1 and secValue>=MIN_CLIP_REG_MATED_RATIO*support_num) mate_reg_num = 2;
		...
		if(orient_num_arr[0][maxIdx]>orient_num_arr[1][maxIdx]){ // mate at left end
			...
			orient_str = "-";
			if(meanClipPos-1>=1) mate_clip_pos_str = to_string(meanClipPos-1);
			else mate_clip_pos_str = "1";
		}
		if(support_num>cov_num) cov_num = support_num;
		if(mate_reg_num==1) left_mate_str = vec_id_str + orient_str + "|" + mate_clip_pos_str + "|" + to_string(support_num) + "|" + to_string(cov_num);
		else left_mate_str = vec_id_str + orient_str + "|" + mate_clip_pos_str + "|" + to_string(maxValue) + "|" + to_string(cov_num);

		// the second mate region
		if(mate_reg_num==2){
			//cout << "secIdx=" << secIdx << ", secValue=" << secValue << ", orient_nums=[" << orient_num_arr[0][secIdx] << "," << orient_num_arr[1][secIdx] << "]" << endl;

			vec_id_str = to_string(secIdx);
			if(orient_num_arr[0][secIdx]>orient_num_arr[1][secIdx]){ // mate at left end
				orient_str = "+";
				mate_clip_pos_str = to_string(meanClipPos);
			}else{ // mate at right end
				orient_str = "-";
			...
			mate_str2 = vec_id_str + orient_str + "|" + mate_clip_pos_str + "|" + to_string(secValue) + "|" + to_string(cov_num);

			left_mate_str = left_mate_str + "_" + mate_str2;
		}
	}else
		left_mate_str = "-";
...
//	cout << num_arr[3] << endl;
	...
	if(maxIdx!=-1 and maxValue>=MIN_CLIP_REG_MATED_RATIO*support_num){
		...
		if(support_num>cov_num) cov_num = support_num;
		if(mate_reg_num==1) right_mate_str = vec_id_str + orient_str + "|" + mate_clip_pos_str + "|" + to_string(support_num) + "|" + to_string(cov_num);
		else right_mate_str = vec_id_str + orient_str + "|" + mate_clip_pos_str + "|" + to_string(maxValue) + "|" + to_string(cov_num);

		if(mate_reg_num==2){
			//cout << "secIdx=" << secIdx << ", secValue=" << secValue << ", orient_nums=[" << orient_num_arr[0][secIdx] << "," << orient_num_arr[1][secIdx] << "]" << endl;

			vec_id_str = to_string(secIdx);
			if(orient_num_arr[0][secIdx]>orient_num_arr[1][secIdx]){ // mate at right end
				orient_str =  "+";
				if(meanClipPos-1>=1) mate_clip_pos_str = to_string(meanClipPos-1);
				else mate_clip_pos_str = "1";
			}else{ // mate at left end
			...
			right_mate_str = right_mate_str + "_" + mate_str2;
		}
...
int32_t clipReg::computeCovNumClipPos(string &chrname, int64_t meanClipPos, int32_t clip_end, faidx_t *fai, Paras *paras){
	...
	if(chr_len!=-1){
		...
		//cout << "line=" << __LINE__ << ", " << chrname << ":" << start_pos << "-" << end_pos << ", clipAlnDataVec.size=" << alnDataVector.size() << endl;
...
void clipReg::computeClipRegs(){
	...
	// adjust leftMeanClipPos and leftMeanClipPos2
	if(mate_clip_reg.leftClipReg and mate_clip_reg.leftClipReg->endRefPos-mate_clip_reg.leftClipReg->startRefPos>MIN_CLIP_END_SIZE)
		mate_clip_reg.leftMeanClipPos = mate_clip_reg.leftClipReg->startRefPos;
	if(mate_clip_reg.leftClipReg2 and mate_clip_reg.leftClipReg2->endRefPos-mate_clip_reg.leftClipReg2->startRefPos>MIN_CLIP_END_SIZE)
		mate_clip_reg.leftMeanClipPos2 = mate_clip_reg.leftClipReg2->startRefPos;
	...
	// adjust rightMeanClipPos and rightMeanClipPos2
	if(mate_clip_reg.rightClipReg and mate_clip_reg.rightClipReg->endRefPos-mate_clip_reg.rightClipReg->startRefPos>MIN_CLIP_END_SIZE)
		mate_clip_reg.rightMeanClipPos = mate_clip_reg.rightClipReg->endRefPos;
	if(mate_clip_reg.rightClipReg2 and mate_clip_reg.rightClipReg2->endRefPos-mate_clip_reg.rightClipReg2->startRefPos>MIN_CLIP_END_SIZE)
		mate_clip_reg.rightMeanClipPos2 = mate_clip_reg.rightClipReg2->endRefPos;
...
//	determineLargeIndelReg();
	...
	// determine large indel region
	if(mate_clip_reg.sv_type==VAR_UNC or mate_clip_reg.sv_type==VAR_TRA) {
	//if(mate_clip_reg.sv_type!=VAR_INS or mate_clip_reg.sv_type!=VAR_DEL) { // deleted on 2026-04-01
		determineLargeIndelReg();
		computeLargeIndelTypeClipReg(mate_clip_reg);
	}
	...
	// set ultralarge flag
	setUltraLargeFlag(mate_clip_reg, paras);
}
...
void clipReg::determineLargeIndelReg(){
	...
	vector<int32_t> adjClipAlnSegInfo, indel_num_vec;
	clipPos_t *clip_pos, *clip_pos_tmp1, *clip_pos_tmp2;
	int32_t mate_arr_idx, mate_clip_end_flag, idx_vec, reg_num, same_qname_num;
	int32_t mate_arr_idx_same_chr, mate_clip_end_flag_same_chr, min_dist_same_chr, min_ref_dist_same_chr, valid_num, increase_direction;
	int64_t start_pos_tmp, end_pos_tmp, dist, dist2, min_dist, min_ref_dist;
	bool valid_mate_flag, same_orient_flag, self_overlap_flag, seg_skip_flag, ref_skip_flag, valid_flag;
	int32_t ins_num, del_num, sv_type_tmp, sv_len_tmp;
	int64_t sum_ins, sum_del, mean_size_ins, mean_size_del;
	set<string> qname_vec;
	...
	if(reg_num==1){ // only one clipPos set
		...
		if(mate_clip_reg.leftClipRegNum==1){ // left part
			...
			if(mate_clip_reg.rightClipReg){
				...
				clip_pos_vec1 = &rightClipPosVector;
			}else{
				reg = mate_clip_reg.rightClipReg2;
				clip_pos_vec1 = &rightClipPosVector2;
			}
		}

		// count the number of same query name;
		same_qname_num = 0;
		for(i=0; i<clip_pos_vec1->size(); i++){
			queryname = clip_pos_vec1->at(i)->clip_aln->queryname;
			if(qname_vec.find(queryname)==qname_vec.end()){ // new item
				qname_vec.insert(queryname);
			}else{
				same_qname_num++;
			}
		}
		...
		if(same_qname_num>0){
			copyClipPosVec(*clip_pos_vec1, largeIndelClipPosVector);
			...
			largeIndelClipReg->rescue_flag = false;
	...
	//		mate_clip_reg.large_indel_flag = true;
		...
		for(i=0; i<clip_pos_vec1->size(); i++){
			...
			if(clip_pos->clip_aln->query_checked_flag==true) continue;

			queryname = clip_pos->clip_aln->queryname;
			query_aln_segs = getQueryClipAlnSegsAll(queryname, clipAlnDataVector);  // get query clip align segments

//			if(queryname.compare("SRR8858451.1.31876")==0){
//				cout << "qname=" << queryname << endl;
//			}
			...
			if(mate_arr_idx!=-1){ // mated
				...
				//if(mate_arr_idx_same_chr!=-1 and mate_arr_idx==mate_arr_idx_same_chr and mate_clip_end_flag==mate_clip_end_flag_same_chr){ // deleted on 2026-05-08
				if(mate_arr_idx_same_chr!=-1){
					mate_clip_aln_seg = query_aln_segs.at(mate_arr_idx_same_chr);
					dist = min_dist_same_chr - min_ref_dist_same_chr;
					if(increase_direction==1) dist = -dist;
					...
					if((mate_clip_end_flag_same_chr==LEFT_END and mate_clip_aln_seg->left_aln) or (mate_clip_end_flag_same_chr==RIGHT_END and mate_clip_aln_seg->right_aln)){
						self_overlap_flag = isSegSelfOverlap(clip_pos->clip_aln, mate_clip_aln_seg, MAX_SV_SIZE_USR);
						dist = min_dist_same_chr - min_ref_dist_same_chr;
						if(increase_direction==1) dist = -dist;
						// prefer the segments on the same chromosome
						//if((mate_arr_idx!=mate_arr_idx_same_chr and (abs(min_ref_dist_same_chr)<=MAX_REF_DIST_SAME_CHR or self_overlap_flag)) or (mate_arr_idx==mate_arr_idx_same_chr and abs(min_dist_same_chr)>MAX_REF_DIST_SAME_CHR and abs(dist)>MAX_REF_DIST_SAME_CHR)){ // different chromosomes with near reference distance
						...
						//if(abs(min_ref_dist_same_chr)<=MAX_REF_DIST_SAME_CHR){ // different chromosomes with near reference distance
							...
							if(abs(min_dist_same_chr)>=MAX_REF_DIST_SAME_CHR and abs(min_dist_same_chr)<=maxVarRegSize and abs(min_ref_dist_same_chr)>MAX_REF_DIST_SAME_CHR and abs(min_ref_dist_same_chr)<=maxVarRegSize){
								if(clip_pos->clip_aln->aln_orient==mate_clip_aln_seg->aln_orient and abs(min_dist)>MAX_REF_DIST_SAME_CHR and abs(min_ref_dist)>MAX_REF_DIST_SAME_CHR){
									if(dist>0){
										mate_arr_idx = mate_arr_idx_same_chr;
										mate_clip_end_flag = mate_clip_end_flag_same_chr;
										//seg_skip_num ++;
				...
				if(seg_skip_flag or ref_skip_flag){
					...
					//self_overlap_flag = isSegSelfOverlap(clip_pos->clip_aln, mate_clip_aln_seg, maxVarRegSize); // deleted on 2026-05-08
					clip_pos_items_tmp1 = getClipPosItemsByQueryname(clip_pos->clip_aln->queryname, *clip_pos_vec1);

					valid_mate_flag = false;
					clip_pos_tmp1 = clip_pos_tmp2 = NULL;
					if(clip_pos_items_tmp1.size()==1){ // only one segment in both vector, respectively
						...
						if(clip_pos_vec2 and clip_pos_vec2!=clip_pos_vec1){
							...
							if(clip_pos_items_tmp2.size()==1) {
								valid_mate_flag = true;
								clip_pos_tmp1 = clip_pos;
								clip_pos_tmp2 = clip_pos_items_tmp2.at(0);
							}
						}
					}else if(clip_pos_items_tmp1.size()==2){ // two segments in single vector
						clip_pos_tmp1 = clip_pos_items_tmp1.at(0);
						clip_pos_tmp2 = clip_pos_items_tmp1.at(1);
					}

					if(clip_pos_tmp1 and clip_pos_tmp2){
						if(seg_skip_flag){ // INS
							if(clip_pos_tmp1->clipQueryPos<clip_pos_tmp2->clipQueryPos)
								dist2 = (clip_pos_tmp2->clipQueryPos - clip_pos_tmp1->clipQueryPos) - (clip_pos_tmp2->clipRefPos - clip_pos_tmp1->clipRefPos);
							else
								dist2 = (clip_pos_tmp1->clipQueryPos - clip_pos_tmp2->clipQueryPos) - (clip_pos_tmp1->clipRefPos - clip_pos_tmp2->clipRefPos);
							if(dist2>0) valid_mate_flag = true;
						}else{ // DEL
							if(clip_pos_tmp1->clipRefPos<clip_pos_tmp2->clipRefPos)
								dist2 = (clip_pos_tmp2->clipQueryPos - clip_pos_tmp1->clipQueryPos) - (clip_pos_tmp2->clipRefPos - clip_pos_tmp1->clipRefPos);
							else
								dist2 = (clip_pos_tmp1->clipQueryPos - clip_pos_tmp2->clipQueryPos) - (clip_pos_tmp1->clipRefPos - clip_pos_tmp2->clipRefPos);
							if(dist2<0) valid_mate_flag = true;
					...
					if(same_orient_flag and valid_mate_flag){
						valid_num ++;

						if(dist2>0) { ins_num ++; sum_ins += dist;}
						else { del_num ++; sum_del += dist; }
					}
				}
			}
			for(auto const& item : query_aln_segs) item->query_checked_flag = true;
		}
		resetClipCheckFlag(clipAlnDataVector);
		...
		sv_type_tmp = VAR_UNC;
		sv_len_tmp = 0;
		if(ins_num>0 or del_num>0){
			if(ins_num>=del_num) { sv_type_tmp = VAR_INS; sv_len_tmp = mean_size_ins; }
			else { sv_type_tmp = VAR_DEL; sv_len_tmp = mean_size_del; }
		}

		indel_num_vec = computeSuppNumIntraAlnSeg(chrname, startRefPos, endRefPos, sv_type_tmp, sv_len_tmp, inBamFile, paras->minMapQ, paras->minHighMapQ, paras->max_ultra_high_cov, fai, QC_SIZE_RATIO_MATCH_THRES_INDEL);
		if(indel_num_vec.size()==2){ // two elements returned
			if(ins_num>=del_num){
				ins_num += indel_num_vec.at(0);
				sum_ins += indel_num_vec.at(1);
			}else{
				del_num += indel_num_vec.at(0);
				sum_del += indel_num_vec.at(1);
			}
		}else{ // four elements returned
			ins_num += indel_num_vec.at(0);
			sum_ins += indel_num_vec.at(2);
			del_num += indel_num_vec.at(1);
			sum_del += indel_num_vec.at(3);
		}
		...
		valid_num = max(ins_num, del_num);

#if CLIP_REG_DEBUG
		cout << "line=" << __LINE__ << ", valid_num=" << valid_num << endl;
#endif

		// construct DEL region
		if(valid_num>paras->minReadsNumSupportSV*READS_NUM_SUPPORT_FACTOR){
			reg1 = reg2 = NULL;
			if(leftClipPosVector.size()>0 and mate_clip_reg.leftClipReg){
				clip_pos_vec1 = &leftClipPosVector;
				reg1 = mate_clip_reg.leftClipReg;
				copyClipPosVec(leftClipPosVector, largeIndelClipPosVector);
			}else if(leftClipPosVector2.size()>0 and mate_clip_reg.leftClipReg2){
				clip_pos_vec1 = &leftClipPosVector2;
				reg1 = mate_clip_reg.leftClipReg2;
				copyClipPosVec(leftClipPosVector2, largeIndelClipPosVector);
			}

			if(mate_clip_reg.rightClipRegNum==1){
				if(rightClipPosVector.size()>0 and mate_clip_reg.rightClipReg){
					clip_pos_vec2 = &rightClipPosVector;
					reg2 = mate_clip_reg.rightClipReg;
				}else if(rightClipPosVector2.size()>0 and mate_clip_reg.rightClipReg2){
					clip_pos_vec2 = &rightClipPosVector2;
					reg2 = mate_clip_reg.rightClipReg2;
				}
				if(reg2){
					dist2 = reg2->startRefPos - reg1->startRefPos;
					if(reg1->chrname.compare(reg2->chrname)==0 and abs(dist2)<=maxVarRegSize)
						copyClipPosVec(*clip_pos_vec2, largeIndelClipPosVector);
					else reg2 = NULL;
				}
			}else if(mate_clip_reg.rightClipRegNum==2){
				if(rightClipPosVector.size()>0 and mate_clip_reg.rightClipReg){
					clip_pos_vec2 = &rightClipPosVector;
					reg2 = mate_clip_reg.rightClipReg;
					dist2 = reg2->startRefPos - reg1->startRefPos;
				...
				if(rightClipPosVector2.size()>0 and mate_clip_reg.rightClipReg2){
					clip_pos_vec2 = &rightClipPosVector2;
					reg2 = mate_clip_reg.rightClipReg2;
					dist2 = reg2->startRefPos - reg1->startRefPos;
					if(reg1->chrname.compare(reg2->chrname)==0 and abs(dist2)<=maxVarRegSize)
			...
			if(reg1 and reg2){
				...
				chrname_tmp = reg1->chrname;
				start_pos_tmp = reg1->startRefPos;
				end_pos_tmp = reg1->endRefPos;
				valid_flag = true;
			}

#if CLIP_REG_DEBUG
			cout << "valid_flag=" << valid_flag << "reg1=" << reg1 << ", reg2=" << reg2 << endl;
#endif
...
vector<int32_t> clipReg::getMateMateRegID(int32_t reg_idx, int32_t end_flag, string bnd_mate_reg_strs[4]){
	...
	char ch_orient;
	vector<string> str_vec, str_vec2, mate_reg_id_vec, mate_reg_id_vec2;
	string mate_reg_id_str, mate_reg_id_str2;
	size_t i, j;

	if(bnd_mate_reg_strs[reg_idx].compare("-")!=0){
		// right end
		mate_reg_id_vec = getBNDMateRegStr(reg_idx, end_flag, bnd_mate_reg_strs);
		for(i=0; i<mate_reg_id_vec.size(); i++){
			mate_reg_id_str = mate_reg_id_vec.at(i);
			if(mate_reg_id_str.compare("-")!=0){
				...
				mate_reg_id_vec2 = getBNDMateRegStr(mate_reg_id, mate_end_flag, bnd_mate_reg_strs);
				for(j=0; j<mate_reg_id_vec2.size(); j++){
					mate_reg_id_str2 = mate_reg_id_vec2.at(j);
...
vector<string> clipReg::getBNDMateRegStr(int32_t reg_idx, int32_t end_flag, string bnd_mate_reg_strs[4]){
	vector<string> str_vec, str_vec2, str_vec_tmp, mate_reg_id_vec;
	string bnd_reg_str, bnd_reg_str2, mate_reg_id_str = "-";

	if(reg_idx>=0 and reg_idx<=3){
		if(bnd_mate_reg_strs[reg_idx].compare("-")!=0){
			bnd_reg_str = bnd_reg_str2 = "";
			...
			if(bnd_reg_str.compare("-")!=0){ // 2+|.|.|.
				// if there is two regions
				if(bnd_reg_str.find('_')==string::npos){ // single region
					str_vec2 = split(bnd_reg_str, "|");  // 2+
					mate_reg_id_str = str_vec2.at(0);
					mate_reg_id_vec.push_back(mate_reg_id_str);
				}else{ // two regions
					str_vec_tmp = split(bnd_reg_str, "_");  // '_'
					for(const auto& item : str_vec_tmp){
						str_vec2 = split(item, "|");  // 2+
						mate_reg_id_str = str_vec2.at(0);
						mate_reg_id_vec.push_back(mate_reg_id_str);
					}
				}
...
void clipReg::removeFPClipSingleEnd(mateClipReg_t &mate_clip_reg){
	int64_t dist;
	bool valid_flag;
	vector<string> mate_regid_vec, mate_regid_vec2;
	...
	if(mate_clip_reg.leftClipRegNum==2){
		...
		if(mate_clip_reg.leftClipReg->chrname.compare(mate_clip_reg.leftClipReg2->chrname)==0){
			...
			//if(dist>MAX_DIST_SAME_CLIP_END) {
			if(dist>maxVarRegSize) {
				for(size_t i=0; i<2; i++){
					mate_regid_vec = getBNDMateRegStr(i, LEFT_END, mate_clip_reg.bnd_mate_reg_strs);
					mate_regid_vec2 = getBNDMateRegStr(i, RIGHT_END, mate_clip_reg.bnd_mate_reg_strs);
					if(mate_regid_vec.size()==0 and mate_regid_vec2.size()==0){
						valid_flag = false;
						break;
					}
				}
...
void clipReg::computeVarTypeClipReg(mateClipReg_t &mate_clip_reg){
	...
	int32_t arr_idx, clip_end_flag, mate_clip_end_flag, occ_num;
	bool same_chr_flag, same_orient_flag, self_overlap_flag, same_aln_reg_flag, both_ends_overlap, turn_round_flag; // six features
	bool valid_query_end_flag, valid_query_flag;
	int64_t new_startRefPos, new_endRefPos;
	vector<int64_t> new_margin_pos_vec;
	...
	if(mate_clip_reg.large_indel_flag==false){
		...
		if(mate_clip_reg.reg_mated_flag){ // clippings
			...
			// get new variant margins
			new_margin_pos_vec = getVarMarginsFromMateClipReg(chrname, startRefPos, endRefPos, mate_clip_reg);
			new_startRefPos = new_margin_pos_vec.at(0);
			new_endRefPos = new_margin_pos_vec.at(1);
...
//					if(queryname.compare("SRR8858470.1.117130")==0){
//						cout << "qname=" << queryname << endl;
//					}
					...
					occ_num = getOccNumInVecs(query_aln_segs, leftClipPosVector, leftClipPosVector2, rightClipPosVector, rightClipPosVector2); // added on 2025-09-08
					if(occ_num==0) continue;
					...
					FilteredbyAlignmentSegment(query_aln_segs, chrname, paras->minMapQ, MIN_ALN_SEG_SIZE_CLIPREG);
					if(query_aln_segs.size()==0) continue;

					same_chr_flag = isSameChrome(query_aln_segs);
					//same_orient_flag = isSameOrient(query_aln_segs, MIN_ALN_SIZE_DIFF_ORIENT); // deleted on 2026-02-02
					...
					both_ends_overlap = isBothEndsOverlap(query_aln_segs, new_startRefPos, new_endRefPos);
					turn_round_flag = isTurnRound(query_aln_segs, new_startRefPos, new_endRefPos);

					// predict the variant type
					var_type_tmp = VAR_UNC;
					//if(same_chr_flag and same_orient_flag and self_overlap_flag and same_aln_reg_flag){ // DUP, deleted on 2026-04-09
					if(same_chr_flag and same_orient_flag and both_ends_overlap and turn_round_flag){ // DUP
						...
						dup_type_num ++;
					//}else if(same_chr_flag and same_orient_flag==false and self_overlap_flag==false){// and same_aln_reg_flag){ // INV, deleted on 2026-04-15
						...
						inv_type_num ++;
					//}else if(self_overlap_flag==false and same_aln_reg_flag==false)	{ // TRA, deleted on 2026-04-16
	...
	//			for(j=0; j<query_aln_segs.size(); j++) query_aln_segs.at(j)->query_checked_flag = true;
		...
		mate_clip_reg.supp_num_clip = 0;
		mate_clip_reg.sv_type = var_type;
		switch(mate_clip_reg.sv_type){
			case VAR_DUP:
				mate_clip_reg.supp_num_clip = dup_type_num;
				mate_clip_reg.dup_num = computeDupNumClipReg(dup_num_vec); //compute the dup_num
				break;
			case VAR_INV: mate_clip_reg.supp_num_clip = inv_type_num; break;
			case VAR_TRA: mate_clip_reg.supp_num_clip = tra_type_num; break;
		}
	}else{ // large indels
		computeLargeIndelTypeClipReg(mate_clip_reg);
	}
...
// compute large indel type for clipping region
void clipReg::computeLargeIndelTypeClipReg(mateClipReg_t &mate_clip_reg){
	size_t i, j;
	clipAlnData_t *mate_clip_aln_seg;
	vector<clipAlnData_t*> query_aln_segs;
	string queryname;
	...
	int32_t mate_arr_idx, mate_clip_end_flag, sv_type_tmp, sv_len_tmp;
	int32_t mate_arr_idx_same_chr, mate_clip_end_flag_same_chr, min_dist_same_chr, min_ref_dist_same_chr;
	bool valid_mate_flag, same_orient_flag, self_overlap_flag;
	bool valid_query_flag, seg_skip_flag, ref_skip_flag;
	vector<clipPos_t*> query_pos_vec, *clip_pos_vec2, clip_pos_items_tmp2;
	int32_t ins_num, del_num, dist, orient_factor, valid_num, idx_vec, increase_direction;
	int64_t sum_ins, sum_del, mean_size_ins, mean_size_del, min_dist, min_ref_dist;
...
//			}
			...
			if(mate_arr_idx!=-1){ // mated
				...
				if(seg_skip_flag or ref_skip_flag){
					//cout << "line=" << __LINE__ << ", qname=" << queryname << ", seg_skip_flag=" << seg_skip_flag << ", ref_skip_flag=" << ref_skip_flag << endl;
					mate_clip_aln_seg = query_aln_segs.at(mate_arr_idx);
					...
					if(clip_pos_mate){
						if(clip_pos->aln_orient==clip_pos_mate->aln_orient){ // same orient
							...
							//if(clip_pos->clip_end==RIGHT_END and clip_pos_mate->clip_end==LEFT_END and clip_pos->clip_aln->startRefPos<clip_pos_mate->clip_aln->endRefPos) // deleted on 2026-02-22
							...
							//else if(clip_pos->clip_end==LEFT_END and clip_pos_mate->clip_end==RIGHT_END and clip_pos->clip_aln->endRefPos>clip_pos_mate->clip_aln->startRefPos) // deleted on 2026-02-22
							else if(clip_pos->clip_end==LEFT_END and clip_pos_mate->clip_end==RIGHT_END)
	...
	if(largeIndelClipReg)
		indel_num_vec = computeSuppNumIntraAlnSeg(largeIndelClipReg->chrname, largeIndelClipReg->startRefPos, largeIndelClipReg->endRefPos, sv_type_tmp, sv_len_tmp, inBamFile, paras->minMapQ, paras->minHighMapQ, paras->max_ultra_high_cov, fai, QC_SIZE_RATIO_MATCH_THRES_INDEL);
	else
		indel_num_vec = computeSuppNumIntraAlnSeg(chrname, startRefPos, endRefPos, sv_type_tmp, sv_len_tmp, inBamFile, paras->minMapQ, paras->minHighMapQ, paras->max_ultra_high_cov, fai, QC_SIZE_RATIO_MATCH_THRES_INDEL);
	if(indel_num_vec.size()==2){ // two elements returned
		if(ins_num>=del_num){
...
#if CLIP_REG_DEBUG
		cout << "line=" << __LINE__ << ", valid_num=" << valid_num << ", supp_num_clip=" << mate_clip_reg.supp_num_clip << ", mean_size_ins=" << mean_size_ins << ", mean_size_del=" << mean_size_del << endl;
#endif

	if(valid_num>=int32_t(paras->minReadsNumSupportSV*READS_NUM_SUPPORT_FACTOR)){
		...
		if((sv_type_tmp==VAR_INS or sv_type_tmp==VAR_DEL) and valid_num>=mate_clip_reg.supp_num_clip){
			...
			mate_clip_reg.supp_num_largeIndel = valid_num;
			supp_num_largeIndel = valid_num;
	...
	if(mate_clip_reg.large_indel_flag==false){ // insufficient supporting reads
...
/**
 * Get the variant margins from clipPosVecs
 */
vector<int64_t> clipReg::getVarMarginsFromMateClipReg(string &chrname, int64_t startRefPos, int64_t endRefPos, mateClipReg_t &mate_clip_reg){
	vector<int64_t> new_margin_pos_vec;
	int64_t min_start_rpos, max_end_rpos;

	min_start_rpos = startRefPos;
	max_end_rpos = endRefPos;

	if(mate_clip_reg.leftClipReg and mate_clip_reg.leftClipReg->chrname.compare(chrname)==0){
		if(mate_clip_reg.leftMeanClipPos<min_start_rpos) min_start_rpos = mate_clip_reg.leftMeanClipPos;
		else if(mate_clip_reg.leftMeanClipPos>max_end_rpos) max_end_rpos = mate_clip_reg.leftMeanClipPos;
	}
	if(mate_clip_reg.leftClipReg2 and mate_clip_reg.leftClipReg2->chrname.compare(chrname)==0){
		if(mate_clip_reg.leftMeanClipPos2<min_start_rpos) min_start_rpos = mate_clip_reg.leftMeanClipPos2;
		else if(mate_clip_reg.leftMeanClipPos2>max_end_rpos) max_end_rpos = mate_clip_reg.leftMeanClipPos2;
	}
	if(mate_clip_reg.rightClipReg and mate_clip_reg.rightClipReg->chrname.compare(chrname)==0){
		if(mate_clip_reg.rightMeanClipPos<min_start_rpos) min_start_rpos = mate_clip_reg.rightMeanClipPos;
		else if(mate_clip_reg.rightMeanClipPos>max_end_rpos) max_end_rpos = mate_clip_reg.rightMeanClipPos;
	}
	if(mate_clip_reg.rightClipReg2 and mate_clip_reg.rightClipReg2->chrname.compare(chrname)==0){
		if(mate_clip_reg.rightMeanClipPos2<min_start_rpos) min_start_rpos = mate_clip_reg.rightMeanClipPos2;
		else if(mate_clip_reg.rightMeanClipPos2>max_end_rpos) max_end_rpos = mate_clip_reg.rightMeanClipPos2;
	}

	new_margin_pos_vec.push_back(min_start_rpos);
	new_margin_pos_vec.push_back(max_end_rpos);

	return new_margin_pos_vec;
}

/**
 * Calculate the support number
 * Two elements returned: the target supp_num and the summed size; four elements returned: the INS and DEL occs and the summed sizes
 */
vector<int32_t> clipReg::computeSuppNumIntraAlnSeg(string &chrname, size_t startRefPos, size_t endRefPos, int32_t var_type, int32_t sv_len, string &inBamFile, int32_t minMapQ, int32_t minHighMapQ, double max_ultra_high_cov, faidx_t *fai, double size_match_ratio_thres){
	vector<int32_t> indel_num_vec;
	...
	char *seq;
	double val, size_sum;
	int32_t ins_num, del_num;
	int64_t supp_sum, ins_sum, del_sum;
	bool overlap_flag;

	supp_num = ins_num = del_num = supp_sum = ins_sum = del_sum = 0;
				...
				if(var_type!=VAR_UNC){
					...
					for(i=0; i<alnDataVector.size(); i++){
						...
						if(!(b->core.flag & BAM_FUNMAP)){ // aligned
							...
							for(j=0; j<alnSegs.size(); j++){
								seg = alnSegs.at(j);
								if((seg->opflag==BAM_CINS or seg->opflag==BAM_CDEL) and seg->seglen>=paras->min_sv_size_usr){
									end_pos_tmp = getEndRefPosAlnSeg(seg->startRpos, seg->opflag, seg->seglen);
									overlap_flag = isOverlappedPos(seg->startRpos, end_pos_tmp, startRefPos, endRefPos);
									if(seg->opflag==BAM_CINS and overlap_flag){
										ins_num ++; ins_sum += seg->seglen;
									}else if(seg->opflag==BAM_CDEL and overlap_flag){
										del_num ++; del_sum += seg->seglen;
									}
								}
							}
					...
					if(ins_num>=del_num) supp_num = ins_num;
					else supp_num = del_num;
				}
			}
		}
	...
	if(var_type!=VAR_UNC){ // only report the target supp_num
		indel_num_vec.push_back(supp_num);
		if(var_type==VAR_INS) indel_num_vec.push_back(supp_sum);
		else indel_num_vec.push_back(-supp_sum);
	}else{ // report the INS and DEL occs
		indel_num_vec.push_back(ins_num);
		indel_num_vec.push_back(del_num);
		indel_num_vec.push_back(ins_sum);
		indel_num_vec.push_back(-del_sum);
	}

	return indel_num_vec;
}
...
size_t clipReg::extractVarType(mateClipReg_t &mate_clip_reg, size_t dup_type_num, size_t inv_type_num, size_t tra_type_num, size_t min_reads_thres){
	...
	int64_t left_pos1, left_pos2, right_pos1, right_pos2, dist;
	bool overlap_flag;
	...
	if(var_type==VAR_TRA and chrname1.compare(chrname2)==0){ // validation, added on 2025-09-19
		left_pos1 = left_pos2 = right_pos1 = right_pos2 = -1;
		if(mate_clip_reg.leftClipRegNum==1){
			if(mate_clip_reg.leftClipReg) {
				left_pos1 = left_pos2 = mate_clip_reg.leftMeanClipPos;
			}else if(mate_clip_reg.leftClipReg2){
				left_pos1 = left_pos2 = mate_clip_reg.leftMeanClipPos2;
			}
		}else if(mate_clip_reg.leftClipRegNum==2){
			left_pos1 = mate_clip_reg.leftMeanClipPos;
			left_pos2 = mate_clip_reg.leftMeanClipPos2;
		}

		if(mate_clip_reg.rightClipRegNum==1){
			if(mate_clip_reg.rightClipReg) {
				right_pos1 = right_pos2 = mate_clip_reg.rightMeanClipPos;
			}else if(mate_clip_reg.rightClipReg2){
				right_pos1 = right_pos2 = mate_clip_reg.rightMeanClipPos2;
			}
		}else if(mate_clip_reg.rightClipRegNum==2){
			right_pos1 = mate_clip_reg.rightMeanClipPos;
			right_pos2 = mate_clip_reg.rightMeanClipPos2;
		}

		overlap_flag = isOverlappedPos(left_pos1, left_pos2, right_pos1, right_pos2);
		if(overlap_flag) var_type = VAR_UNC;
		else{
			dist = right_pos1 - left_pos2;
			if(abs(dist)<MIN_REF_DIST_TRA) var_type = VAR_UNC;
		}
	}else if(var_type==VAR_INV and mate_clip_reg.leftClipRegNum!=mate_clip_reg.rightClipRegNum){ // validation, added on 2025-10-23
		var_type = VAR_UNC;
	}
